...
if __name__ == "__main__": 
    ...
    for caller, callees in call_graph.items():
        if caller in pickle_dict:
            optional_callees = [
                c for c in callees
                if c in pickle_dict and c != caller
            ]
            if optional_callees:
                transaction = set(
                    [caller] + optional_callees)
                transactions.append(transaction)

    print("Transactions found: {}".format(
        len(transactions)))

    # checking the association rules
    min_confidence = 0.5
    func_count = defaultdict(int)
    for transaction in transactions:
        for func in transaction:
            func_count[func] += 1

    cooccurrence = defaultdict(lambda: defaultdict(int))
    for transaction in transactions:
        funcs = list(transaction)
        for i in range(len(funcs)):
            for j in range(len(funcs)):
                if i != j:
                    cooccurrence[funcs[i]][funcs[j]] += 1

    rule_partners = defaultdict(set)
    for funcA in pickle_dict:
        if func_count[funcA] > 0:
            for funcB in pickle_dict:
                if funcA != funcB and \
                        cooccurrence[funcA][funcB] > 0:
                    confidence = (
                        cooccurrence[funcA][funcB] /
                        func_count[funcA]
                    )
                    if confidence >= min_confidence:
                        rule_partners[funcA].add(funcB)

    associated = set()
    for func, partners in rule_partners.items():
        if partners:
            associated.add(func)
            associated.update(partners)

    isolated = set(pickle_dict.keys()) - associated

    print("Functions with associations: {}".format(
        len(associated)))
    ...
    sorted_associated = sorted(
        associated,
        key=lambda f: len(rule_partners.get(f, set())),
        reverse=True
    )

    current_assoc_group = []

    for func in sorted_associated:
        if func in visited:
            continue
        ...
        for partner in rule_partners.get(func, set()):
            if partner not in visited and \
                ...
                current_assoc_group.append(partner)
                visited.add(partner)

        # When group reaches max_size save it
        if len(current_assoc_group) >= max_size:
            gid = "hybrid_assoc_{}".format(group_id)
            groups[gid] = {
                f: pickle_dict[f]
                for f in current_assoc_group
                if f in pickle_dict
            }
            for f in current_assoc_group:
                if f in pickle_dict:
    ...
    for func, count in sorted_isolated:
        current_group.append(func)
        if len(current_group) == freq_group_size:
            gid = "hybrid_freq_{}".format(freq_id)
            groups[gid] = {
                f: pickle_dict[f]
    ...
    # Save all groups as plain JSON
    output_dir = os.path.dirname(unused_gzip_dir)
    if not output_dir:
        output_dir = sta_file
    ...
    index_file = os.path.join(
        output_dir, "hybrid_group_index.json")
    with open(index_file, 'w') as f:
        json.dump(group_index, f, indent=2)

    print("\nComplete")
    print("Association groups: hybrid_assoc_*.json")
    print("Frequency groups:   hybrid_freq_*.json")
