...
if __name__ == "__main__": 
    ...
    for func_name, count in sorted_functions:
        ...
        if len(current_group) == 10:
            ...
            groups[gid] = {
                f: pickle_dict[f] for f in current_group
            }
    ...
    index_file = os.path.join(output_dir, "group_index.json")
    with open(index_file, 'w') as f:
        json.dump(group_index, f, indent=2)

    print("\nImprovement Complete")
    print("Total groups created: {}".format(len(groups)))
    print("Total optional functions grouped: {}".format(
        len(pickle_dict)))
    print("Plain JSON groups saved directly")