...
from botocore.utils import PRIORITY_ORDERED_SUPPORTED_PROTOCOLS, ensure_boolean, is_s3_accelerate_url
...
VALID_REQUEST_CHECKSUM_CALCULATION_CONFIG = ('when_supported', 'when_required')
VALID_RESPONSE_CHECKSUM_VALIDATION_CONFIG = ('when_supported', 'when_required')
VALID_ACCOUNT_ID_ENDPOINT_MODE_CONFIG = ('preferred', 'disabled', 'required')
...
class ClientArgsCreator:
    ...
    def get_client_args(self, service_model, region_name, is_secure, endpoint_url, verify, credentials, scoped_config, client_config, endpoint_bridge, auth_token=None, endpoints_ruleset_data=None, partition_data=None):
        ...
        s3_disable_express_session_auth = config_kwargs['s3_disable_express_session_auth']
        ...
        event_emitter.emit(event_name, protocol_name=protocol, service_model=service_model, serializer_kwargs=serializer_kwargs)
        serializer = botocore.serialize.create_serializer(protocol, parameter_validation, timestamp_precision=serializer_kwargs['timestamp_precision'])
        response_parser = botocore.parsers.create_parser(protocol)
        ruleset_resolver = self._build_endpoint_resolver(endpoints_ruleset_data, partition_data, client_config, service_model, endpoint_region_name, region_name, configured_endpoint_url, endpoint, is_secure, endpoint_bridge, event_emitter, credentials, account_id_endpoint_mode, s3_disable_express_session_auth, auth_scheme_preference)
    ...
    def _validate_s3_disable_express_session_auth(self, config_val):
        string_bool = (isinstance(config_val, str) and config_val.lower() in ['true', 'false'])
        if (not isinstance(config_val, bool) and not string_bool):
            raise botocore.exceptions.InvalidConfigError(error_msg=f'Invalid value "{config_val}" for s3_disable_express_session_auth. Value must be a boolean')
    ...
    def _compute_s3_disable_express_session_auth(self, config_kwargs):
        ...
        config_kwargs['s3_disable_express_session_auth'] = ensure_boolean(disable_express)
    ...
    def _build_endpoint_resolver(self, endpoints_ruleset_data, partition_data, client_config, service_model, endpoint_region_name, region_name, endpoint_url, endpoint, is_secure, endpoint_bridge, event_emitter, credentials, account_id_endpoint_mode, s3_disable_express_session_auth, auth_scheme_preference):
        ...
        if self._is_s3_service(service_name_raw):
            ...
            if s3_disable_express_session_auth is not None:
                client_context['disable_s3_express_session_auth'] = s3_disable_express_session_auth
        sig_version = (client_config.signature_version if client_config is not None else None)
        return EndpointRulesetResolver(endpoint_ruleset_data=endpoints_ruleset_data, partition_data=partition_data, service_model=service_model, builtins=resolver_builtins, client_context=client_context, event_emitter=event_emitter, use_ssl=is_secure, requested_auth_scheme=sig_version, auth_scheme_preference=auth_scheme_preference)
    
    def compute_endpoint_resolver_builtin_defaults(self, region_name, service_name, s3_config, endpoint_bridge, client_endpoint_url, legacy_endpoint_url, credentials, account_id_endpoint_mode, s3_disable_express_session_auth):
    ...
    def _compute_user_agent_appid_config(self, config_kwargs):
        ...
        if (user_agent_appid is not None and len(user_agent_appid) > USERAGENT_APPID_MAXLEN):
            logger.warning('The configured value for user_agent_appid exceeds the maximum length of %d characters.', USERAGENT_APPID_MAXLEN)
    ...
    def _compute_checksum_config(self, config_kwargs):
        self._handle_checksum_config(config_kwargs, config_key='request_checksum_calculation', valid_options=VALID_REQUEST_CHECKSUM_CALCULATION_CONFIG)
        self._handle_checksum_config(config_kwargs, config_key='response_checksum_validation', valid_options=VALID_RESPONSE_CHECKSUM_VALIDATION_CONFIG)
    
    def _handle_checksum_config(self, config_kwargs, config_key, valid_options):
        ...
        if value not in valid_options:
            raise botocore.exceptions.InvalidChecksumConfigError(config_key=config_key, config_value=value, valid_options=valid_options)
    ...
    def _compute_account_id_endpoint_mode_config(self, config_kwargs):
        ...
        if account_id_endpoint_mode not in VALID_ACCOUNT_ID_ENDPOINT_MODE_CONFIG:
            raise botocore.exceptions.InvalidConfigError(error_msg=f"The configured value '{account_id_endpoint_mode}' for '{config_key}' is invalid. Valid values are: {VALID_ACCOUNT_ID_ENDPOINT_MODE_CONFIG}.")
    ...
    def _compute_auth_scheme_preference_config(self, client_config, config_kwargs):
        ...
        if not isinstance(value, str):
            raise botocore.exceptions.InvalidConfigError(error_msg=f'{config_key} must be a comma-delimited string. Received {type(value)} instead: {value}.')
