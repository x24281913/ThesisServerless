...
class WaiterDocumenter:
    ...
    def __init__(self, client, service_waiter_model, root_docs_path):
        ...
        self._USER_GUIDE_LINK = 'https://docs.aws.amazon.com/boto3/latest/guide/clients.html#waiters'
...
def document_wait_method(section, waiter_name, event_emitter, service_model, service_waiter_model, include_signature=True):
    ...
    waiter_config_members['Delay'] = DocumentedShape(name='Delay', type_name='integer', documentation=f'<p>The amount of time in seconds to wait between attempts. Default: {waiter_model.delay}</p>')
    waiter_config_members['MaxAttempts'] = DocumentedShape(name='MaxAttempts', type_name='integer', documentation=f'<p>The maximum number of attempts to be made. Default: {waiter_model.max_attempts}</p>')
    ...
    wait_description = f'Polls :py:meth:`{get_service_module_name(service_model)}.Client.{xform_name(waiter_model.operation)}` every {waiter_model.delay} seconds until a successful state is reached. An error is raised after {waiter_model.max_attempts} failed checks.'
