...
class HTTPClientError(BotoCoreError):
    ...
    def __reduce__(self):
        return (_exception_from_packed_args, (self.__class__, (self.request, self.response), self.kwargs))
...
class UnknownFIPSEndpointError(BaseEndpointResolverError):
    ...
    :ivar region_name: The name of the region.
    """
    fmt = 'The provided FIPS pseudo-region "{region_name}" is not known for the service "{service_name}". A FIPS compliant endpoint cannot be constructed.'
...
class ClientError(Exception):
    ...
    def __init__(self, error_response, operation_name):
        ...
        msg = self.MSG_TEMPLATE.format(error_code=error.get('Code', 'Unknown'), error_message=error.get('Message', 'Unknown'), operation_name=operation_name, retry_info=retry_info)
        super().__init__(msg)
...
class LoginError(BotoCoreError):
    fmt = 'An unspecified error happened when resolving AWS credentials or refreshing a login session profile.'
...
class LoginInsufficientPermissions(LoginError):
    fmt = "Unable to create or refresh login credentials due to insufficient permissions. You may be missing permission for the 'signin:CreateOAuth2Token' action."
