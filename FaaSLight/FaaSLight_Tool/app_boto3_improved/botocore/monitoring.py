...
class Monitor:
    _EVENTS_TO_REGISTER = ['before-parameter-build', 'request-created', 'response-received', 'after-call', 'after-call-error']
    ...
    def capture(self, event_name, **payload):
        ...
        try:
            ...
            logger.debug('Exception %s raised by client monitor in handling event %s', e, event_name, exc_info=True)
...
class SocketPublisher:
    ...
    def publish(self, event):
        ...
        if len(serialized_event) > self._MAX_MONITOR_EVENT_LENGTH:
            logger.debug('Serialized event of size %s exceeds the maximum length allowed: %s. Not sending event to socket.', len(serialized_event), self._MAX_MONITOR_EVENT_LENGTH)
            return
