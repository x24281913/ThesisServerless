...
from botocore.utils import has_header, is_json_value_header, parse_to_aware_datetime, percent_encode
DEFAULT_TIMESTAMP_FORMAT = 'iso8601'
ISO8601 = '%Y-%m-%dT%H:%M:%SZ'
ISO8601_MICRO = '%Y-%m-%dT%H:%M:%S.%fZ'
HOST_PREFIX_RE = re.compile('^[A-Za-z0-9\\.\\-]+$')
TIMESTAMP_PRECISION_DEFAULT = 'default'
TIMESTAMP_PRECISION_MILLISECOND = 'millisecond'
TIMESTAMP_PRECISION_OPTIONS = (TIMESTAMP_PRECISION_DEFAULT, TIMESTAMP_PRECISION_MILLISECOND)

def create_serializer(protocol_name, include_validation=True, timestamp_precision=TIMESTAMP_PRECISION_DEFAULT):
...
class Serializer:
    ...
    def __init__(self, timestamp_precision=TIMESTAMP_PRECISION_DEFAULT):
        if timestamp_precision not in TIMESTAMP_PRECISION_OPTIONS:
            raise ValueError(f'Invalid timestamp precision found while creating serializer: {timestamp_precision}')
    ...
    def _create_default_request(self):
        serialized = {'url_path': '/', 'query_string': '', 'method': self.DEFAULT_METHOD, 'headers': {}, 'body': b''}
        return serialized
...
class JSONSerializer(Serializer):
    ...
    def serialize_to_request(self, parameters, operation_model):
        ...
        serialized = self._create_default_request()
        serialized['method'] = operation_model.http.get('method', self.DEFAULT_METHOD)
        serialized['headers'] = {'X-Amz-Target': target, 'Content-Type': f'application/x-amz-json-{json_version}'}
...
class CBORSerializer(Serializer):
    ...
    def _is_special_number(self, value):
        return any([value == float('inf'), value == float('-inf'), math.isnan(value)])
    ...
    def _get_bytes_for_special_numbers(self, value):
        ...
        if value == float('inf'):
            return initial_byte + struct.pack('>H', 31744)
        elif value == float('-inf'):
            return initial_byte + struct.pack('>H', 64512)
        elif math.isnan(value):
            return initial_byte + struct.pack('>H', 32256)
...
SERIALIZERS = {'ec2': EC2Serializer, 'query': QuerySerializer, 'json': JSONSerializer, 'rest-json': RestJSONSerializer, 'rest-xml': RestXMLSerializer, 'smithy-rpc-v2-cbor': RpcV2CBORSerializer}
