...
class Session:
    ...
    def get_service_data(self, service_name, api_version=None):
        ...
        self._events.emit(f'service-data-loaded.{service_id}', service_data=service_data, service_name=service_name, session=self)
        return service_data
    ...
    def create_client(self, service_name, region_name=None, api_version=None, use_ssl=True, verify=None, endpoint_url=None, aws_access_key_id=None, aws_secret_access_key=None, aws_session_token=None, config=None, aws_account_id=None):
        ...
        if (config is not None and config.signature_version is UNSIGNED):
            ...
            credentials = botocore.credentials.Credentials(access_key=aws_access_key_id, secret_key=aws_secret_access_key, token=aws_session_token, account_id=aws_account_id)
        elif self._missing_cred_vars(aws_access_key_id, aws_secret_access_key):
            raise PartialCredentialsError(provider='explicit', cred_var=self._missing_cred_vars(aws_access_key_id, aws_secret_access_key))
        else:
        ...
        user_agent_creator = self.get_component('user_agent_creator')
        user_agent_creator.set_session_config(session_user_agent_name=self.user_agent_name, session_user_agent_version=self.user_agent_version, session_user_agent_extra=self.user_agent_extra)
        ...
        self._add_configured_endpoint_provider(client_name=service_name, config_store=config_store)
        user_agent_creator.set_client_features(get_context().features)
        client_creator = botocore.client.ClientCreator(loader, endpoint_resolver, self.user_agent(), event_emitter, retryhandler, translate, response_parser_factory, exceptions_factory, config_store, user_agent_creator=user_agent_creator, auth_token_resolver=self.get_auth_token)
        client = client_creator.create_client(service_name=service_name, region_name=region_name, is_secure=use_ssl, endpoint_url=endpoint_url, verify=verify, credentials=credentials, scoped_config=self.get_scoped_config(), client_config=config, api_version=api_version, auth_token=auth_token)
