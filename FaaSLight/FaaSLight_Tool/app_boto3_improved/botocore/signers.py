...
from botocore.exceptions import ParamValidationError, UnknownClientMethodError, UnknownSignatureVersionError, UnsupportedSignatureVersionError
...
class RequestSigner:
    ...
    * before-sign: Allows mutating the request before signing.
    ...
    def sign(self, operation_name, request, region_name=None, signing_type='standard', expires_in=None, signing_name=None):
        ...
        self._event_emitter.emit(f'before-sign.{self._service_id.hyphenize()}.{operation_name}', request=request, signing_name=signing_name, region_name=self._region_name, signature_version=signature_version, request_signer=self, operation_name=operation_name)
        if signature_version != botocore.UNSIGNED:
            kwargs = {'signing_name': signing_name, 'region_name': region_name, 'signature_version': signature_version}
            ...
            if signing_context.get('identity_cache') is not None:
                self._resolve_identity_cache(kwargs, signing_context['identity_cache'], signing_context['cache_key'])
            try:
    ...
    def _choose_signer(self, operation_name, signing_type, context):
        ...
        (handler, response) = self._event_emitter.emit_until_response(f'choose-signer.{self._service_id.hyphenize()}.{operation_name}', signing_name=signing_name, region_name=region_name, signature_version=signature_version, context=context)
    ...
    def get_auth_instance(self, signing_name, region_name, signature_version=None, request_credentials=None, **kwargs):
        """
    ...
    def generate_presigned_url(self, request_dict, operation_name, expires_in=3600, region_name=None, signing_name=None):
...
def _dsql_generate_db_auth_token(self, Hostname, Action, Region=None, ExpiresIn=900):
    ...
    if Region is None:
        Region = self.meta.region_name
    request_dict = {'url_path': '/', 'query_string': '', 'headers': {}, 'body': {'Action': Action}, 'method': 'GET'}
    scheme = 'https://'
    endpoint_url = f'{scheme}{Hostname}'
    prepare_request_dict(request_dict, endpoint_url)
    presigned_url = self._request_signer.generate_presigned_url(operation_name=Action, request_dict=request_dict, region_name=Region, expires_in=ExpiresIn, signing_name='dsql')
    return presigned_url[len(scheme):]
...
def generate_presigned_url(self, ClientMethod, Params=None, ExpiresIn=3600, HttpMethod=None):
    ...
    context = {'is_presign_request': True, 'use_global_endpoint': _should_use_global_endpoint(self)}
    ...
    params = self._emit_api_params(api_params=params, operation_model=operation_model, context=context)
    bucket_is_arn = ArnParser.is_arn(params.get('Bucket', ''))
    (endpoint_url, additional_headers, properties) = self._resolve_endpoint_ruleset(operation_model, params, context, ignore_signing_region=not bucket_is_arn)
    request_dict = self._convert_to_request_dict(api_params=params, operation_model=operation_model, endpoint_url=endpoint_url, context=context, headers=additional_headers, set_user_agent_header=False)
...
def generate_presigned_post(self, Bucket, Key, Fields=None, Conditions=None, ExpiresIn=3600):
    ...
    params = self._emit_api_params(api_params={'Bucket': bucket}, operation_model=operation_model, context=context)
    bucket_is_arn = ArnParser.is_arn(params.get('Bucket', ''))
    (endpoint_url, additional_headers, properties) = self._resolve_endpoint_ruleset(operation_model, params, context, ignore_signing_region=not bucket_is_arn)
    request_dict = self._convert_to_request_dict(api_params=params, operation_model=operation_model, endpoint_url=endpoint_url, context=context, headers=additional_headers, set_user_agent_header=False)
