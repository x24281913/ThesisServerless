...
from botocore.exceptions import ParamValidationError, StubAssertionError, StubResponseError, UnStubbedResponseError
...
class Stubber:
    ...
    **Example:**
    ...
    def activate(self):
        ...
        self.client.meta.events.register('before-endpoint-resolution.*', self._set_account_id_for_endpoint_resolution, unique_id=self._stub_account_id_event_id)
    ...
    def deactivate(self):
        ...
        self.client.meta.events.unregister('before-endpoint-resolution.*', self._stub_account_id_event_id, unique_id=self._stub_account_id_event_id)
    ...
    def _assert_expected_params(self, model, params, context, **kwargs):
        ...
        for (param, value) in expected_params.items():
            if (param not in params or expected_params[param] != params[param]):
                raise StubAssertionError(operation_name=model.name, reason=f'Expected parameters:\n{pformat(expected_params)},\nbut received:\n{pformat(params)}')
