...
from botocore.compat import HAS_CRT, HEX_PAT, IPV4_PAT, IPV4_RE, IPV6_ADDRZ_PAT, IPV6_ADDRZ_RE, IPV6_PAT, LS32_PAT, MD5_AVAILABLE, UNRESERVED_PAT, UNSAFE_URL_CHARS, ZONE_ID_PAT, OrderedDict, get_current_datetime, get_md5, get_tzinfo_options, json, quote, urlparse, urlsplit, urlunsplit, zip_longest
    ...
    def _evaluate_expiration(self, credentials):
        ...
        try:
            ...
            if current_time >= extension_time:
                ...
                logger.info('Attempting credential expiration extension due to a credential service availability issue. A refresh of these credentials will be attempted again within the next %.0f minutes.', refresh_interval_with_jitter / 60)
        except ValueError:
...
class S3RegionRedirectorv2:
    ...
    def redirect_from_error(self, request_dict, response, operation, **kwargs):
        ...
        if not any([is_special_head_object, is_wrong_signing_region, is_permanent_redirect, is_special_head_bucket, is_redirect_status, is_opt_in_region_redirect]):
            return
...
class S3EndpointSetter:
    ...
    def set_endpoint(self, request, **kwargs):
        ...
        if self._use_accelerate_endpoint:
            if self._use_fips_endpoint:
                raise UnsupportedS3ConfigurationError(msg=f'Client is configured to use the FIPS psuedo region for "{self._region}", but S3 Accelerate does not have any FIPS compatible endpoints.')
    ...
    def _validate_fips_supported(self, request):
        ...
        if 'outpost_name' in request.context['s3_accesspoint']:
            raise UnsupportedS3AccesspointConfigurationError(msg=f'Client is configured to use the FIPS psuedo-region "{self._region}", but outpost ARNs do not support FIPS endpoints.')
        ...
        if accesspoint_region != self._region:
            if not self._s3_config.get('use_arn_region', True):
                raise UnsupportedS3AccesspointConfigurationError(msg=f'Client is configured to use the FIPS psuedo-region for "{self._region}", but the access-point ARN provided is for the "{accesspoint_region}" region. For clients using a FIPS psuedo-region calls to access-point ARNs in another region are not allowed.')
    ...
    def _validate_global_regions(self, request):
        ...
        if self._region in ['aws-global', 's3-external-1']:
            raise UnsupportedS3AccesspointConfigurationError(msg=f'Client is configured to use the global psuedo-region "{self._region}". When providing access-point ARNs a regional endpoint must be specified.')
    ...
    def _validate_accesspoint_supported(self, request):
        ...
        if request_partition != self._partition:
            raise UnsupportedS3AccesspointConfigurationError(msg=f'Client is configured for "{self._partition}" partition, but access-point ARN provided is for "{request_partition}" partition. The client and  access-point partition must be the same.')
    ...
    def _validate_endpoint_from_arn_details_supported(self, request):
        ...
        if not self._s3_config.get('use_arn_region', False):
            ...
            if arn_region != self._region:
                error_msg = f'The use_arn_region configuration is disabled but received arn for "{arn_region}" when the client is configured to use "{self._region}"'
        ...
        if request_partion != self._partition:
            raise UnsupportedS3ControlConfigurationError(msg=f'Client is configured for "{self._partition}" partition, but arn provided is for "{request_partion}" partition. The client and arn partition must be the same.')
        if self._s3_config.get('use_accelerate_endpoint'):
...
class S3ControlArnParamHandler:
    ...
    def __init__(self, arn_parser=None):
        ...
        if arn_parser is None:
            self._arn_parser = ArnParser()
        warnings.warn('The S3ControlArnParamHandler class has been deprecated for a new internal replacement. A future version of botocore may remove this class.', category=FutureWarning)
    
    def register(self, event_emitter):
        event_emitter.register('before-parameter-build.s3-control', self.handle_arn)
