        ...
        try:
            ...
            for i in range(max_attempts):
                try:
                    ...
                    return
                except (socket.timeout, OSError, ReadTimeoutError, IncompleteReadError, ResponseStreamingError) as e:
...
class TransferConfig:
    ...
    def __init__(self, multipart_threshold=8 * MB, max_concurrency=10, multipart_chunksize=8 * MB, num_download_attempts=5, max_io_queue=100):
...
class S3Transfer:
    ...
    ALLOWED_UPLOAD_ARGS = ['ACL', 'CacheControl', 'ContentDisposition', 'ContentEncoding', 'ContentLanguage', 'ContentType', 'Expires', 'GrantFullControl', 'GrantRead', 'GrantReadACP', 'GrantWriteACL', 'Metadata', 'RequestPayer', 'ServerSideEncryption', 'StorageClass', 'SSECustomerAlgorithm', 'SSECustomerKey', 'SSECustomerKeyMD5', 'SSEKMSKeyId', 'SSEKMSEncryptionContext', 'Tagging']
    ...
    def _put_object(self, filename, bucket, key, callback, extra_args):
        ...
        with open_chunk_reader(filename, 0, self._osutil.get_file_size(filename), callback=callback) as body:
    ...
    def download_file(self, bucket, key, filename, extra_args=None, callback=None):
        ...
        try:
            ...
            logger.debug('Exception caught in download_file, removing partial file: %s', temp_filename, exc_info=True)
