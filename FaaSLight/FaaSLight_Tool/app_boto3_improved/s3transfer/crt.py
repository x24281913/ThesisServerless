...
from awscrt.auth import AwsCredentials, AwsCredentialsProvider, AwsSigningAlgorithm, AwsSigningConfig
...
from s3transfer.utils import CallArgs, OSUtils, create_nested_client, get_callbacks, is_s3express_bucket
logger = logging.getLogger(__name__)
CRT_S3_PROCESS_LOCK = None

def acquire_crt_s3_process_lock(name):
    import custom_funtemplate
    return custom_funtemplate.rewrite_template('s3transfer.crt.acquire_crt_s3_process_lock', 'acquire_crt_s3_process_lock(name)', {'awscrt': awscrt, 'name': name}, 1)
...
def create_s3_crt_client(region, crt_credentials_provider=None, num_threads=None, target_throughput=None, part_size=8 * MB, use_ssl=True, verify=None):
    ...
    """
    import custom_funtemplate
    return custom_funtemplate.rewrite_template('s3transfer.crt.create_s3_crt_client', 'create_s3_crt_client(region, crt_credentials_provider=None, num_threads=None, target_throughput=None, part_size=8 * MB, use_ssl=True, verify=None)', {'EventLoopGroup': EventLoopGroup, 'DefaultHostResolver': DefaultHostResolver, 'ClientBootstrap': ClientBootstrap, 'S3RequestTlsMode': S3RequestTlsMode, 'TlsContextOptions': TlsContextOptions, 'ClientTlsContext': ClientTlsContext, '_get_crt_throughput_target_gbps': _get_crt_throughput_target_gbps, 'S3Client': S3Client, 'region': region, 'crt_credentials_provider': crt_credentials_provider, 'num_threads': num_threads, 'target_throughput': target_throughput, 'part_size': part_size, 'use_ssl': use_ssl, 'verify': verify}, 1)

def _get_crt_throughput_target_gbps(provided_throughput_target_bytes=None):
    import custom_funtemplate
    return custom_funtemplate.rewrite_template('s3transfer.crt._get_crt_throughput_target_gbps', '_get_crt_throughput_target_gbps(provided_throughput_target_bytes=None)', {'awscrt': awscrt, 'logger': logger, 'provided_throughput_target_bytes': provided_throughput_target_bytes}, 1)

def _has_minimum_crt_version(minimum_version):
    import custom_funtemplate
    return custom_funtemplate.rewrite_template('s3transfer.crt._has_minimum_crt_version', '_has_minimum_crt_version(minimum_version)', {'awscrt': awscrt, 'minimum_version': minimum_version}, 1)
...
class CRTTransferManager:
    ...
    def __init__(self, crt_s3_client, crt_request_serializer, osutil=None, config=None):
        ...
        self._s3_args_creator = S3ClientArgsCreator(crt_request_serializer, self._osutil, config)
...
class S3ClientArgsCreator:
    _CRT_ARG_TO_CONFIG_PARAM = {'max_active_connections_override': CRTConfigParameter('max_request_concurrency', (0, 29, 0))}
    ...
    def _get_crt_transfer_config_options(self, request_type):
        crt_config = {'part_size': self._config.multipart_chunksize, 'max_active_connections_override': self._config.max_request_concurrency}
        if self._config.get_deep_attr('multipart_chunksize') is self._config.UNSET_DEFAULT:
            crt_config['part_size'] = None
        if self._config.get_deep_attr('max_request_concurrency') is self._config.UNSET_DEFAULT:
    ...
    def _remove_param_if_not_min_crt_version(self, crt_config):
        ...
        for request_arg in crt_config:
            ...
            if self._config.get_deep_attr(param.name) is not self._config.UNSET_DEFAULT:
                ...
                logger.warning(f'Transfer config parameter {param.name} requires minimum CRT version: {min_ver_str}. {param.name} will not be used in the request.')
    ...
    def _get_make_request_args_put_object(self, request_type, call_args, coordinator, future, on_done_before_calls, on_done_after_calls):
        ...
        if self._config is not None:
            make_request_args.update(self._get_crt_transfer_config_options(request_type))
    ...
    def _default_get_make_request_args(self, request_type, call_args, coordinator, future, on_done_before_calls, on_done_after_calls):
        ...
        if accesspoint_arn_details := (arn_handler.handle_arn(call_args.bucket) and accesspoint_arn_details['region'] == ''):
            make_request_args['signing_config'] = AwsSigningConfig(algorithm=AwsSigningAlgorithm.V4_ASYMMETRIC, region='*', use_double_uri_encode=False, should_normalize_uri_path=False)
            call_args.bucket = accesspoint_arn_details['resource_name']
        elif is_s3express_bucket(call_args.bucket):
            make_request_args['signing_config'] = AwsSigningConfig(algorithm=AwsSigningAlgorithm.V4_S3EXPRESS, use_double_uri_encode=False, should_normalize_uri_path=False)
        return make_request_args
