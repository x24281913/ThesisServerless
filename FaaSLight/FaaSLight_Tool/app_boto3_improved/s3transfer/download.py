...
from s3transfer.exceptions import RetriesExceededError, S3DownloadFailedError, S3ValidationError
from s3transfer.futures import IN_MEMORY_DOWNLOAD_TAG
from s3transfer.tasks import SubmissionTask, Task
from s3transfer.utils import S3_RETRYABLE_DOWNLOAD_ERRORS, CountCallbackInvoker, DeferredOpenFile, FunctionContainer, StreamReaderProgress, calculate_num_parts, calculate_range_parameter, get_callbacks, invoke_progress_callbacks
    ...
    def _main(self, client, bucket, key, fileobj, extra_args, callbacks, max_attempts, download_output_manager, io_chunksize, start_index=0, bandwidth_limiter=None):
        ...
        for i in range(max_attempts):
            try:
                ...
                self._validate_content_range(extra_args.get('Range'), response.get('ContentRange'))
...
class CompleteDownloadNOOPTask(Task):
    ...
    def __init__(self, transfer_coordinator, main_kwargs=None, pending_main_kwargs=None, done_callbacks=None, is_final=True):
        super().__init__(transfer_coordinator=transfer_coordinator, main_kwargs=main_kwargs, pending_main_kwargs=pending_main_kwargs, done_callbacks=done_callbacks, is_final=is_final)
