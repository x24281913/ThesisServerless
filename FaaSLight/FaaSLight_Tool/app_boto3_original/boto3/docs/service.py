...
class ServiceDocumenter(BaseServiceDocumenter):
    ...
    def __init__(self, service_name, session, root_docs_path):
        super().__init__(service_name=service_name, session=session._session, root_docs_path=root_docs_path)
        ...
        self.sections = ['title', 'client', 'paginators', 'waiters', 'resources', 'examples', 'context-params']
        self._root_docs_path = root_docs_path
        self._USER_GUIDE_LINK = 'https://docs.aws.amazon.com/boto3/latest/guide/resources.html'
    ...
    def _document_service_resource(self, section):
        ...
        service_resource_dir_path = os.path.join(self._root_docs_path, f'{self._service_name}', f'{resource_name.lower()}')
    ...
    def _document_resources(self, section):
        ...
        for resource_name in json_resource_model['resources']:
            ...
            ResourceDocumenter(resource, self._session, self._root_docs_path).document_resource(resource_doc.add_new_section(resource.meta.resource_model.name))
            service_resource_dir_path = os.path.join(self._root_docs_path, f'{self._service_name}', f'{resource_name}')
            resource_doc.write_to_file(service_resource_dir_path, 'index')
            section.style.tocitem(f'{self._service_name}/{resource_name}/index')
    
    def _get_example_file(self):
        return os.path.realpath(os.path.join(self.EXAMPLE_PATH, f'{self._service_name}.rst'))
