...
from botocore.compat import HAS_CRT, MD5_AVAILABLE, HTTPHeaders, encodebytes, ensure_unicode, get_current_datetime, parse_qs, quote, unquote, urlsplit, urlunsplit
from botocore.exceptions import NoAuthTokenError, NoCredentialsError, UnknownSignatureVersionError, UnsupportedSignatureVersionError
...
SIGNED_HEADERS_BLACKLIST = ['connection', 'expect', 'keep-alive', 'proxy-authenticate', 'proxy-authorization', 'te', 'trailer', 'transfer-encoding', 'upgrade', 'user-agent', 'x-amzn-trace-id']
...
AUTH_TYPE_MAPS = {'v2': SigV2Auth, 'v3': SigV3Auth, 'v3https': SigV3Auth, 's3': HmacV1Auth, 's3-query': HmacV1QueryAuth, 's3-presign-post': HmacV1PostAuth, 's3v4-presign-post': S3SigV4PostAuth, 'v4-s3express': S3ExpressAuth, 'v4-s3express-query': S3ExpressQueryAuth, 'v4-s3express-presign-post': S3ExpressPostAuth, 'bearer': BearerAuth}
if HAS_CRT:
    from botocore.crt.auth import CRT_AUTH_TYPE_MAPS
    AUTH_TYPE_MAPS.update(CRT_AUTH_TYPE_MAPS)
else:
    AUTH_TYPE_MAPS.update({'v4': SigV4Auth, 'v4-query': SigV4QueryAuth, 's3v4': S3SigV4Auth, 's3v4-query': S3SigV4QueryAuth})
AUTH_TYPE_TO_SIGNATURE_VERSION = {'aws.auth#sigv4': 'v4', 'aws.auth#sigv4a': 'v4a', 'smithy.api#httpBearerAuth': 'bearer', 'smithy.api#noAuth': 'none'}
