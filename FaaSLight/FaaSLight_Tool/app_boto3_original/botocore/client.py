...
from botocore.auth import AUTH_TYPE_MAPS, resolve_auth_scheme_preference, resolve_auth_type
from botocore.awsrequest import prepare_request_dict
from botocore.compress import maybe_compress_request
from botocore.config import Config
from botocore.context import with_current_context
from botocore.credentials import RefreshableCredentials
from botocore.discovery import EndpointDiscoveryHandler, EndpointDiscoveryManager, block_endpoint_discovery_required_operations
from botocore.docs.docstring import ClientMethodDocstring, PaginatorDocstring
from botocore.exceptions import ClientError, DataNotFoundError, InvalidEndpointDiscoveryConfigurationError, OperationNotPageableError, UnknownServiceError, UnknownSignatureVersionError
...
from botocore.utils import CachedProperty, EventbridgeSignerSetter, S3ArnParamHandler, S3ControlArnParamHandler, S3ControlArnParamHandlerv2, S3ControlEndpointSetter, S3EndpointSetter, S3ExpressIdentityResolver, S3RegionRedirector, S3RegionRedirectorv2, ensure_boolean, get_service_module_name
...
class ClientCreator:
    ...
    def __init__(self, loader, endpoint_resolver, user_agent, event_emitter, retry_handler_factory, retry_config_translator, response_parser_factory=None, exceptions_factory=None, config_store=None, user_agent_creator=None, auth_token_resolver=None):
        self._loader = loader
    ...
    def create_client(self, service_name, region_name, is_secure=True, endpoint_url=None, verify=None, credentials=None, scoped_config=None, api_version=None, client_config=None, auth_token=None):
        ...
        endpoint_bridge = ClientEndpointBridge(self._endpoint_resolver, scoped_config, client_config, service_signing_name=service_model.metadata.get('signingName'), config_store=self._config_store, service_signature_version=service_signature_version)
    ...
    def _create_client_class(self, service_name, service_model):
        ...
        self._event_emitter.emit(f'creating-client-class.{service_id}', class_attributes=class_attributes, base_classes=bases)
    ...
    def _register_importexport_events(self, client, endpoint_bridge=None, endpoint_url=None, client_config=None, scoped_config=None):
    ...
    def _create_api_method(self, py_operation_name, operation_name, service_model):
        ...
        docstring = ClientMethodDocstring(operation_model=operation_model, method_name=operation_name, event_emitter=self._event_emitter, method_description=operation_model.documentation, example_prefix=f'response = client.{py_operation_name}', include_signature=False)
...
class BaseClient:
    ...
    def __init__(self, serializer, endpoint, response_parser, event_emitter, request_signer, service_model, loader, client_config, partition, exceptions_factory, endpoint_ruleset_resolver=None, user_agent_creator=None):
        ...
        self.meta = ClientMeta(event_emitter, self._client_config, endpoint.host, service_model, self._PY_TO_OP_NAME, partition)
        self._exceptions_factory = exceptions_factory
    ...
    def _register_handlers(self):
        ...
        self.meta.events.register_last(f'request-created.{service_id}', self._user_agent_creator.rebuild_and_replace_user_agent_handler)
    ...
    def _make_api_call(self, operation_name, api_params):
        ...
        history_recorder.record('API_CALL', {'service': service_name, 'operation': operation_name, 'params': api_params})
        ...
        if (operation_model.auth and self.meta.config.auth_scheme_preference):
            ...
            auth_type = resolve_auth_scheme_preference(preferred_schemes, operation_model.auth)
        else:
            auth_type = operation_model.resolved_auth_type
        request_context = {'client_region': self.meta.region_name, 'client_config': self.meta.config, 'has_streaming_input': operation_model.has_streaming_input, 'auth_type': auth_type, 'unsigned_payload': operation_model.unsigned_payload, 'auth_options': self._service_model.metadata.get('auth')}
        api_params = self._emit_api_params(api_params=api_params, operation_model=operation_model, context=request_context)
        (endpoint_url, additional_headers, properties) = self._resolve_endpoint_ruleset(operation_model, api_params, request_context)
        if properties:
            request_context['endpoint_properties'] = properties
        request_dict = self._convert_to_request_dict(api_params=api_params, operation_model=operation_model, endpoint_url=endpoint_url, context=request_context, headers=additional_headers)
        resolve_checksum_context(request_dict, operation_model, api_params)
        service_id = self._service_model.service_id.hyphenize()
        (handler, event_response) = self.meta.events.emit_until_response(f'before-call.{service_id}.{operation_name}', model=operation_model, params=request_dict, request_signer=self._request_signer, context=request_context)
        if event_response is not None:
        ...
        self.meta.events.emit(f'after-call.{service_id}.{operation_name}', http_response=http, parsed=parsed_response, model=operation_model, context=request_context)
    ...
    def _make_request(self, operation_model, request_dict, request_context):
        try:
            ...
            self.meta.events.emit(f'after-call-error.{self._service_model.service_id.hyphenize()}.{operation_model.name}', exception=e, context=request_context)
            raise
