...
from botocore.exceptions import ConfigNotFound, CredentialRetrievalError, InfiniteLoopConfigError, InvalidConfigError, LoginError, LoginInsufficientPermissions, LoginRefreshRequired, LoginTokenLoadError, MetadataRetrievalError, MissingDependencyException, PartialCredentialsError, RefreshWithMFAUnsupportedError, UnauthorizedSSOTokenError, UnknownCredentialError
from botocore.tokens import SSOTokenProvider
from botocore.useragent import register_feature_id, register_feature_ids
from botocore.utils import ArnParser, ContainerMetadataFetcher, FileWebIdentityTokenLoader, InstanceMetadataFetcher, JSONFileCache, LoginTokenLoader, SSOTokenLoader, create_nested_client, get_login_token_cache_directory, parse_key_val_file, resolve_imds_endpoint_mode
logger = logging.getLogger(__name__)
ReadOnlyCredentials = namedtuple('ReadOnlyCredentials', ['access_key', 'secret_key', 'token', 'account_id'], defaults=(None, ))
_DEFAULT_MANDATORY_REFRESH_TIMEOUT = 10 * 60
_DEFAULT_ADVISORY_REFRESH_TIMEOUT = 15 * 60
...
class ProfileProviderBuilder:
    ...
    def __init__(self, session, cache=None, region_name=None, sso_token_cache=None, login_token_cache=None):
    ...
    def providers(self, profile_name, disable_env_vars=False):
        return [self._create_web_identity_provider(profile_name, disable_env_vars), self._create_sso_provider(profile_name), self._create_shared_credential_provider(profile_name), self._create_login_provider(profile_name), self._create_process_provider(profile_name), self._create_config_provider(profile_name)]
    ...
    def _create_login_provider(self, profile_name):
        return LoginProvider(load_config=lambda: self._session.full_config, client_creator=self._session.create_client, profile_name=profile_name, token_cache=self._login_token_cache)
...
class RefreshableCredentials(Credentials):
    ...
    def __init__(self, access_key, secret_key, token, expiry_time, refresh_using, method, time_fetcher=_local_now, advisory_timeout=None, mandatory_timeout=None, account_id=None):
    ...
    def create_from_metadata(cls, metadata, refresh_using, method, advisory_timeout=None, mandatory_timeout=None):
        ...
        instance = cls(access_key=metadata['access_key'], secret_key=metadata['secret_key'], token=metadata['token'], expiry_time=cls._expiry_datetime(metadata['expiry_time']), method=method, refresh_using=refresh_using, account_id=metadata.get('account_id'), **kwargs)
        return instance
...
class CachedCredentialFetcher:
    ...
    def _get_cached_credentials(self):
        ...
        credentials = {'access_key': creds['AccessKeyId'], 'secret_key': creds['SecretAccessKey'], 'token': creds['SessionToken'], 'expiry_time': expiration, 'account_id': creds.get('AccountId')}
        return credentials
...
class ProcessProvider(CredentialProvider):
    ...
    def load(self):
        ...
        return Credentials(access_key=creds_dict['access_key'], secret_key=creds_dict['secret_key'], token=creds_dict.get('token'), method=self.METHOD, account_id=creds_dict.get('account_id'))
    ...
    def _retrieve_credentials_using(self, credential_process):
        ...
        try:
            return {'access_key': parsed['AccessKeyId'], 'secret_key': parsed['SecretAccessKey'], 'token': parsed.get('SessionToken'), 'expiry_time': parsed.get('Expiration'), 'account_id': self._get_account_id(parsed)}
    ...
    def load(self):
        ...
        if access_key:
            ...
            if expiry_time is not None:
                ...
                return RefreshableCredentials(credentials['access_key'], credentials['secret_key'], credentials['token'], expiry_time, refresh_using=fetcher, method=self.METHOD, account_id=credentials['account_id'])
            return Credentials(credentials['access_key'], credentials['secret_key'], credentials['token'], method=self.METHOD, account_id=credentials['account_id'])
        else:
            return None
    ...
    def load(self):
        ...
        if self._profile_name in available_creds:
            ...
            if self.ACCESS_KEY in config:
                ...
                return Credentials(access_key, secret_key, token, method=self.METHOD, account_id=account_id)
...
class AssumeRoleProvider(CredentialProvider):
    ...
    NAMED_PROVIDER_FEATURE_MAP = {'Ec2InstanceMetadata': 'CREDENTIALS_IMDS', 'Environment': 'CREDENTIALS_ENV_VARS', 'EcsContainer': 'CREDENTIALS_HTTP'}
    ...
    def _resolve_credentials_from_source(self, credential_source, profile_name):
        ...
        if credentials is None:
            raise CredentialRetrievalError(provider=credential_source, error_msg=f'No credentials found in credential_source referenced in profile {profile_name}')
...
class ContainerProvider(CredentialProvider):
    ...
    def _retrieve_or_fail(self):
        ...
        creds = fetcher()
        return RefreshableCredentials(access_key=creds['access_key'], secret_key=creds['secret_key'], token=creds['token'], method=self.METHOD, expiry_time=_parse_if_needed(creds['expiry_time']), refresh_using=fetcher, account_id=creds.get('account_id'))
    ...
    def _create_fetcher(self, full_uri, *args, **kwargs):
        ...
        def fetch_creds():
            ...
            return {'access_key': response['AccessKeyId'], 'secret_key': response['SecretAccessKey'], 'token': response['Token'], 'expiry_time': response['Expiration'], 'account_id': response.get('AccountId')}
        return fetch_creds
...
class SSOCredentialFetcher(CachedCredentialFetcher):
    ...
    def __init__(self, start_url, sso_region, role_name, account_id, client_creator, token_loader=None, cache=None, expiry_window_seconds=None, token_provider=None, sso_session_name=None, time_fetcher=_local_now):
        self._client_creator = client_creator
    ...
    def _get_credentials(self):
        ...
        credentials = {'ProviderType': 'sso', 'Credentials': {'AccessKeyId': credentials['accessKeyId'], 'SecretAccessKey': credentials['secretAccessKey'], 'SessionToken': credentials['sessionToken'], 'Expiration': self._parse_timestamp(credentials['expiration']), 'AccountId': self._account_id}}
        return credentials
...
class SSOProvider(CredentialProvider):
    ...
    def _load_sso_config(self):
        ...
        if missing_config_vars:
            ...
            raise InvalidConfigError(error_msg=f'The profile "{profile_name}" is configured to use SSO but is missing required configuration: {missing}')
        return config
...
def _build_dpop_header(private_key, uri, uid=None, ts=None):
    if EC is None:
        raise MissingDependencyException(msg='This operation requires an additional dependency. You will need to pip install "botocore[crt]" before proceeding.')
    (x, y) = private_key.get_public_coords()
    jwk = {'kty': 'EC', 'x': _base64_url_encode_no_padding(x), 'y': _base64_url_encode_no_padding(y), 'crv': 'P-256'}
    header = {'typ': 'dpop+jwt', 'alg': 'ES256', 'jwk': jwk}
    payload = {'htm': 'POST', 'htu': uri, 'iat': (ts or int(time.time())), 'jti': (uid or str(uuid.uuid4()))}
    header_b64 = _base64_url_encode_no_padding(json.dumps(header, separators=(',', ':')).encode())
    payload_b64 = _base64_url_encode_no_padding(json.dumps(payload, separators=(',', ':')).encode())
...
def _build_add_dpop_header_handler(private_key):
    ...
    def _add_dpop_header_handler(**kwargs):
        kwargs['params']['headers']['DPoP'] = _build_dpop_header(private_key, kwargs['params']['url'])
...
class LoginCredentialFetcher:
    ...
    _REQUIRED_TOKEN_FIELDS = ('accessToken', 'refreshToken', 'dpopKey', 'clientId')
    
    def __init__(self, session_name, token_loader, client_creator, time_fetcher=_local_now, feature_ids=None):
    ...
    def load_cached_credentials(self):
        ...
        if token is None:
            raise LoginTokenLoadError(error_msg=f"Unable to load a existing login session for session {self._session_name}. Please reauthenticate with 'aws login'.")
        missing_fields = [key for key in self._REQUIRED_TOKEN_FIELDS if key not in token]
        if missing_fields:
            raise LoginTokenLoadError(error_msg=f"Failed to load access token from token cache, missing required fields: {', '.join(missing_fields)}.")
    ...
    def refresh_credentials(self):
        ...
        client = self._client_creator('signin', config=config)
        client.meta.events.register('before-call.signin.CreateOAuth2Token', _build_add_dpop_header_handler(private_key))
        try:
            response = client.create_o_auth2_token(tokenInput={'clientId': token['clientId'], 'refreshToken': token['refreshToken'], 'grantType': 'refresh_token'})
        ...
        if (response is None or 'tokenOutput' not in response):
            raise LoginTokenLoadError(error_msg="Unable to refresh access token due to an invalid service response. Please try running 'aws login' again. If the issue persists, there may be a temporary signin service problem.")
        output = response.get('tokenOutput')
        expires_timestamp = self._time_fetcher().astimezone(tzutc()) + datetime.timedelta(seconds=output['expiresIn'])
        token.update({'accessToken': {'accessKeyId': output['accessToken']['accessKeyId'], 'secretAccessKey': output['accessToken']['secretAccessKey'], 'sessionToken': output['accessToken']['sessionToken'], 'accountId': token['accessToken']['accountId'], 'expiresAt': expires_timestamp.strftime('%Y-%m-%dT%H:%M:%SZ')}, 'refreshToken': output['refreshToken']})
        self._token_loader.save_token(self._session_name, token)
        return self._token_to_credentials(token)
    
    @staticmethod
    def _token_to_credentials(token):
        return {'access_key': token['accessToken']['accessKeyId'], 'secret_key': token['accessToken']['secretAccessKey'], 'token': token['accessToken']['sessionToken'], 'expiry_time': token['accessToken']['expiresAt'], 'account_id': token['accessToken']['accountId']}
    
    @staticmethod
    def _load_private_key(token):
        if 'dpopKey' not in token:
            raise LoginTokenLoadError(error_msg='Private key not found in cached token.')
        lines = token['dpopKey'].splitlines()
        content_lines = [line for line in lines if (not line.startswith('-----BEGIN') and not line.startswith('-----END'))]
        contents = ''.join(content_lines).strip()
        try:
            return EC.new_key_from_der_data(base64.b64decode(contents))
        except ValueError as e:
            raise LoginTokenLoadError(error_msg='Unable to load private key from cached token.') from e
...
class LoginProvider(CredentialProvider):
    ...
    def __init__(self, load_config, client_creator, profile_name, token_cache=None):
        super().__init__()
        if token_cache is None:
    ...
    def load(self):
        ...
        if 'login_session' not in profile_config:
            return None
        if EC is None:
            raise MissingDependencyException(msg='Using the login credential provider requires an additional dependency. You will need to pip install "botocore[crt]" before proceeding.')
        fetcher = LoginCredentialFetcher(session_name=profile_config['login_session'], token_loader=LoginTokenLoader(self._token_cache), client_creator=self._client_creator, time_fetcher=_local_now, feature_ids=self._feature_ids)
        register_feature_ids(self._feature_ids)
        cached_credentials = fetcher.load_cached_credentials()
        return RefreshableCredentials(access_key=cached_credentials['access_key'], secret_key=cached_credentials['secret_key'], token=cached_credentials['token'], expiry_time=_parse_if_needed(cached_credentials['expiry_time']), account_id=cached_credentials['account_id'], method=self.METHOD, refresh_using=fetcher.refresh_credentials, time_fetcher=_local_now)
