...
from botocore.utils import get_environ_proxies, is_valid_endpoint_url, is_valid_ipv6_endpoint_url
...
class Endpoint:
    ...
    def __init__(self, host, endpoint_prefix, event_emitter, response_parser_factory=None, http_session=None):
    ...
    def make_request(self, operation_model, request_dict):
        logger.debug('Making request for %s with params: %s', operation_model, request_dict)
        return self._send_request(request_dict, operation_model)
    
    def create_request(self, params, operation_model=None):
        request = create_request_object(params)
        if operation_model:
            request.stream_output = any([operation_model.has_streaming_output, operation_model.has_event_stream_output])
    ...
    def _send_request(self, request_dict, operation_model):
        ...
        while self._needs_retry(attempts, operation_model, request_dict, success_response, exception):
            attempts += 1
    ...
    def _get_response(self, request, operation_model, context):
        ...
        kwargs_to_emit = {'response_dict': None, 'parsed_response': None, 'context': context, 'exception': exception}
        ...
        service_id = operation_model.service_model.service_id.hyphenize()
        self._event_emitter.emit(f'response-received.{service_id}.{operation_model.name}', **kwargs_to_emit)
    ...
    def _do_get_response(self, request, operation_model, context):
        ...
        self._event_emitter.emit(f'before-parse.{service_id}.{operation_model.name}', operation_model=operation_model, response_dict=response_dict, customized_response_dict=customized_response_dict)
...
class EndpointCreator:
    ...
    def create_endpoint(self, service_model, region_name, endpoint_url, verify=None, response_parser_factory=None, timeout=DEFAULT_TIMEOUT, max_pool_connections=MAX_POOL_CONNECTIONS, http_session_cls=URLLib3Session, proxies=None, socket_options=None, client_cert=None, proxies_config=None):
        ...
        http_session = http_session_cls(timeout=timeout, proxies=proxies, verify=self._get_verify_value(verify), max_pool_connections=max_pool_connections, socket_options=socket_options, client_cert=client_cert, proxies_config=proxies_config)
        return Endpoint(endpoint_url, endpoint_prefix=endpoint_prefix, event_emitter=self._event_emitter, response_parser_factory=response_parser_factory, http_session=http_session)
