...
from botocore.utils import ArnParser, InvalidArnException, is_valid_ipv4_endpoint_url, is_valid_ipv6_endpoint_url, lru_cache_weakref, normalize_url_path, percent_encode
logger = logging.getLogger(__name__)
TEMPLATE_STRING_RE = re.compile('\\{[a-zA-Z#]+\\}')
GET_ATTR_RE = re.compile('(\\w*)\\[(\\d+)\\]')
VALID_HOST_LABEL_RE = re.compile('^(?!-)[a-zA-Z\\d-]{1,63}(?<!-)$')
CACHE_SIZE = 100
...
class ParameterDefinition:
    ...
    def __init__(self, name, parameter_type, documentation=None, builtIn=None, default=None, required=None, deprecated=None):
        self.name = name
        try:
            self.parameter_type = getattr(ParameterType, parameter_type.lower()).value
        except AttributeError:
            raise EndpointResolutionError(msg=f'Unknown parameter type: {parameter_type}. A parameter must be of type string, boolean, or stringarray.')
        self.documentation = documentation
