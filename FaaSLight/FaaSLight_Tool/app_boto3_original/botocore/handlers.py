...
from botocore.compat import MD5_AVAILABLE, ETree, OrderedDict, XMLParseError, ensure_bytes, get_md5, json, quote, unquote, unquote_str, urlsplit, urlunsplit
from botocore.docs.utils import AppendParamDocumentation, AutoPopulatedParam, HideParamFromOperations
from botocore.endpoint_provider import VALID_HOST_LABEL_RE
from botocore.exceptions import AliasConflictParameterError, MissingServiceIdError, ParamValidationError, UnsupportedTLSVersionWarning
from botocore.regions import EndpointResolverBuiltins
from botocore.serialize import TIMESTAMP_PRECISION_MILLISECOND
from botocore.signers import add_dsql_generate_db_auth_token_methods, add_generate_db_auth_token, add_generate_presigned_post, add_generate_presigned_url
from botocore.useragent import register_feature_id
from botocore.utils import SAFE_CHARS, SERVICE_NAME_ALIASES, ArnParser, get_token_from_environment, hyphenize_service_id, is_global_accesspoint, percent_encode, switch_host_with_param
logger = logging.getLogger(__name__)
REGISTER_FIRST = object()
REGISTER_LAST = object()
VALID_BUCKET = re.compile('^[a-zA-Z0-9.\\-_]{1,255}$')
_ACCESSPOINT_ARN = '^arn:(aws).*:(s3|s3-object-lambda):[a-z\\-0-9]*:[0-9]{12}:accesspoint[/:][a-zA-Z0-9\\-.]{1,63}$'
_OUTPOST_ARN = '^arn:(aws).*:s3-outposts:[a-z\\-0-9]+:[0-9]{12}:outpost[/:][a-zA-Z0-9\\-]{1,63}[/:]accesspoint[/:][a-zA-Z0-9\\-]{1,63}$'
VALID_S3_ARN = re.compile('|'.join([_ACCESSPOINT_ARN, _OUTPOST_ARN]))
S3_SIGNING_NAMES = ('s3', 's3-outposts', 's3-object-lambda', 's3express')
VERSION_ID_SUFFIX = re.compile('\\?versionId=[^\\s]+$')
...
def add_expect_header(model, params, **kwargs):
    ...
    if 'body' in params:
        ...
        if hasattr(body, 'read'):
            check_body = utils.ensure_boolean(os.environ.get('BOTO_EXPERIMENTAL__NO_EMPTY_CONTINUE', False))
...
def validate_ascii_metadata(params, **kwargs):
    ...
    for (key, value) in metadata.items():
        try:
            ...
            error_msg = f'Non ascii characters found in S3 metadata for key "{key}", value: "{value}".  \nS3 metadata can only contain ASCII characters. '
            raise ParamValidationError(report=error_msg)
...
def check_openssl_supports_tls_version_1_2(**kwargs):
    ...
    try:
        ...
        if openssl_version_tuple < (1, 0, 1):
            warnings.warn(f'Currently installed openssl version: {ssl.OPENSSL_VERSION} does not support TLS 1.2, which is required for use of iot-data. Please use python installed with openssl version 1.0.1 or higher.', UnsupportedTLSVersionWarning)
    ...
    if _has_expires_shape(operation_model.output_shape):
        if expires_value := response_dict.get('headers', {}).get('Expires'):
            ...
            try:
                ...
                logger.warning('Failed to parse the "Expires" member as a timestamp: %s. The unparsed value is available in the response under "ExpiresString".', expires_value)
...
def _handle_200_error(operation_model, response_dict, **kwargs):
    ...
    if _looks_like_special_case_error(response_dict['status_code'], response_dict['body']):
        ...
        logger.debug('Error found for response with 200 status code: %s.', response_dict['body'])
...
def _set_auth_scheme_preference_signer(context, signing_name, **kwargs):
    ...
    if _should_prefer_bearer_auth(has_in_code_configuration, signing_name, resolved_signature_version, auth_options):
...
def _should_prefer_bearer_auth(has_in_code_configuration, signing_name, resolved_signature_version, auth_options):
