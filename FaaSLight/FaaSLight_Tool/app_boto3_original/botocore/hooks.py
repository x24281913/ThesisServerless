...
class HierarchicalEmitter(BaseEventHooks):
        ...
        if unique_id is not None:
            if unique_id in self._unique_id_handlers:
                ...
                if unique_id_uses_count:
                    if not count:
                        raise ValueError(f'Initial registration of  unique id {unique_id} was specified to use a counter. Subsequent register calls to unique id must specify use of a counter as well.')
                    else:
                        self._unique_id_handlers[unique_id]['count'] += 1
                elif count:
                    raise ValueError(f'Initial registration of unique id {unique_id} was specified to not use a counter. Subsequent register calls to unique id must specify not to use a counter as well.')
                return
            else:
    ...
    def unregister(self, event_name, handler=None, unique_id=None, unique_id_uses_count=False):
        if unique_id is not None:
            ...
            if unique_id_uses_count:
                if count is None:
                    raise ValueError(f'Initial registration of unique id {unique_id} was specified to use a counter. Subsequent unregister calls to unique id must specify use of a counter as well.')
                elif count == 1:
                ...
                if count:
                    raise ValueError(f'Initial registration of unique id {unique_id} was specified to not use a counter. Subsequent unregister calls to unique id must specify not to use a counter as well.')
                handler = self._unique_id_handlers.pop(unique_id)['handler']
        try:
