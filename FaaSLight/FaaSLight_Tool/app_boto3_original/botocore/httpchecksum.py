...
from botocore.exceptions import AwsChunkedWrapperError, FlexibleChecksumError, MissingDependencyException
...
class CrtCrc64NvmeChecksum(BaseChecksum):
    ...
    def update(self, chunk):
        ...
        self._int_crc64nvme = new_checksum & 18446744073709551615
...
def resolve_request_checksum_algorithm(request, operation_model, params, supported_algorithms=None):
    ...
    if (algorithm_member and algorithm_member in params):
        ...
        if algorithm_member_header is not None:
            checksum_context['request_algorithm_header'] = {'name': algorithm_member_header, 'value': DEFAULT_CHECKSUM_ALGORITHM}
    else:
        return
    ...
    algorithm = {'algorithm': algorithm_name, 'in': location_type, 'name': f'x-amz-checksum-{algorithm_name}'}
...
def _register_checksum_feature_ids(request):
    ...
    if algorithm_headers := get_checksum_algorithm_headers(request):
        for header in algorithm_headers:
            header = header.upper()
            if header not in ('X-AMZ-CHECKSUM-ALGORITHM', 'X-AMZ-CHECKSUM-MODE', 'X-AMZ-CHECKSUM-TYPE'):
...
def handle_checksum_body(http_response, response, context, operation_model):
    ...
    for algorithm in algorithms:
        ...
        return
    logger.debug('Skipping checksum validation. Response did not contain one of the following algorithms: %s.', algorithms)
...
def _handle_bytes_response(http_response, response, algorithm):
    ...
    if checksum.digest() != base64.b64decode(expected):
        ...
        raise FlexibleChecksumError(error_msg=error_msg)
    return body
_CHECKSUM_CLS = {'crc32': Crc32Checksum, 'sha1': Sha1Checksum, 'sha256': Sha256Checksum, 'sha512': Sha512Checksum}
_CRT_CHECKSUM_ALGORITHMS = ['crc32', 'crc32c', 'crc64nvme', 'xxhash64', 'xxhash3', 'xxhash128']
if HAS_CRT:
    _CRT_CHECKSUM_CLS = {'crc32': CrtCrc32Checksum, 'crc32c': CrtCrc32cChecksum}
...
_ALGORITHMS_PRIORITY_LIST = ['xxhash128', 'xxhash3', 'crc64nvme', 'xxhash64', 'crc32c', 'crc32', 'sha1', 'sha256', 'sha512']
