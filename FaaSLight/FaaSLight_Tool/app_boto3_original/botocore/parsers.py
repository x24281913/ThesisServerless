...
from botocore.utils import CachedProperty, ensure_boolean, is_json_value_header, lowercase_dict, merge_dicts, parse_timestamp
    ...
    def _has_unknown_tagged_union_member(self, shape, value):
        if shape.is_tagged_union:
            ...
            if tag not in serialized_member_names:
                LOG.info('Received a tagged union response with member unknown to client: %s. Please upgrade SDK for full response support.', tag)
                return True
        return False
...
class BaseCBORParser(ResponseParser):
    ...
    def major_type_to_parsing_method_map(self):
        return {0: self._parse_unsigned_integer, 1: self._parse_negative_integer, 2: self._parse_byte_string, 3: self._parse_text_string, 4: self._parse_array, 5: self._parse_map, 6: self._parse_tag, 7: self._parse_simple_and_float}
    ...
    def parse_data_item(self, stream):
        ...
        additional_info = initial_byte & 31
    ...
    def _parse_unsigned_integer(self, stream, additional_info):
        additional_info_to_num_bytes = {24: 1, 25: 2, 26: 4, 27: 8}
        if additional_info < 24:
            ...
            raise ResponseParserError(f'Invalid CBOR integer returned from the service; unparsable additional info found for major type 0 or 1: {additional_info}')
    ...
    def _parse_simple_and_float(self, stream, additional_info):
        ...
        raise ResponseParserError(f'Invalid additional info found for major type 7: {additional_info}.  This indicates an unsupported simple type or an indefinite float value')
...
class RpcV2CBORParser(BaseRpcV2Parser, BaseCBORParser):
    ...
    def _do_error_parse(self, response, shape):
        body = self._initial_body_parse(response['body'])
        error = {'Error': {'Message': body.get('message', body.get('Message', '')), 'Code': ''}, 'ResponseMetadata': {}}
        headers = response['headers']
...
PROTOCOL_PARSERS = {'ec2': EC2QueryParser, 'query': QueryParser, 'json': JSONParser, 'rest-json': RestJSONParser, 'rest-xml': RestXMLParser, 'smithy-rpc-v2-cbor': RpcV2CBORParser}
