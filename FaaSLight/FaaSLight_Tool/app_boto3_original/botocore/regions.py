...
from botocore.auth import AUTH_TYPE_MAPS, HAS_CRT, resolve_auth_scheme_preference
...
class EndpointResolver(BaseEndpointResolver):
    ...
    def get_available_endpoints(self, service_name, partition_name='aws', allow_non_regional=False, endpoint_variant_tags=None):
        result = []
        for partition in self._endpoint_data['partitions']:
            if partition['partition'] != partition_name:
                continue
    ...
    def construct_endpoint(self, service_name, region_name=None, partition_name=None, use_dualstack_endpoint=False, use_fips_endpoint=False):
        ...
        if partition_name is not None:
            ...
            if valid_partition is not None:
                result = self._endpoint_for_partition(valid_partition, service_name, region_name, use_dualstack_endpoint, use_fips_endpoint, True)
                return result
            return None
        for partition in self._endpoint_data['partitions']:
            if (use_dualstack_endpoint and partition['partition'] in self._UNSUPPORTED_DUALSTACK_PARTITIONS):
                continue
            result = self._endpoint_for_partition(partition, service_name, region_name, use_dualstack_endpoint, use_fips_endpoint)
            if result:
                return result
    ...
    def get_partition_for_region(self, region_name):
        ...
        raise UnknownRegionError(region_name=region_name, error_msg='No partition found for provided region_name.')
    
    def _endpoint_for_partition(self, partition, service_name, region_name, use_dualstack_endpoint, use_fips_endpoint, force_partition=False):
        ...
        if region_name is None:
            if 'partitionEndpoint' in service_data:
                ...
                raise NoRegionError()
        resolve_kwargs = {'partition': partition, 'service_name': service_name, 'service_data': service_data, 'endpoint_name': region_name, 'use_dualstack_endpoint': use_dualstack_endpoint, 'use_fips_endpoint': use_fips_endpoint}
    ...
    def _resolve(self, partition, service_name, service_data, endpoint_name, use_dualstack_endpoint, use_fips_endpoint):
        endpoint_data = service_data.get('endpoints', {}).get(endpoint_name, {})
        if endpoint_data.get('deprecated'):
            LOG.warning('Client is configured with the deprecated endpoint: %s', endpoint_name)
        ...
        result['hostname'] = self._expand_template(partition, result['hostname'], service_name, endpoint_name, result['dnsSuffix'])
        if 'sslCommonName' in result:
            result['sslCommonName'] = self._expand_template(partition, result['sslCommonName'], service_name, endpoint_name, result['dnsSuffix'])
        return result
...
class EndpointRulesetResolver:
    ...
    def __init__(self, endpoint_ruleset_data, partition_data, service_model, builtins, client_context, event_emitter, use_ssl=True, requested_auth_scheme=None, auth_scheme_preference=None):
    ...
    def _get_customized_builtins(self, operation_model, call_args, request_context):
        ...
        self._event_emitter.emit(f'before-endpoint-resolution.{service_id}', builtins=customized_builtins, model=operation_model, params=call_args, context=request_context)
