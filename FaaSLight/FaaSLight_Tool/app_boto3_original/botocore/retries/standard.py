...
import logging
import random
from botocore.exceptions import ConnectionError, ConnectTimeoutError, HTTPClientError, ReadTimeoutError
...
def register_retry_handler(client, max_attempts=DEFAULT_MAX_ATTEMPTS):
    ...
    handler = RetryHandler(retry_policy=RetryPolicy(retry_checker=StandardRetryConditions(max_attempts=max_attempts), retry_backoff=ExponentialBackoff()), retry_event_adapter=RetryEventAdapter(), retry_quota=retry_quota)
    unique_id = f'retry-config-{service_event_name}'
    client.meta.events.register(f'needs-retry.{service_event_name}', handler.needs_retry, unique_id=unique_id)
    return handler
...
class ExponentialBackoff(BaseRetryBackoff):
    ...
    def delay_amount(self, context):
        ...
        """
        return self._random() * min(self._base**(context.attempt_number - 1), self._max_backoff)
