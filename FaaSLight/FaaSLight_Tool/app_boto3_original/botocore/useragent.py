...
class UserAgentComponent(NamedTuple):
    ...
    def to_string(self):
        ...
        if self.size_config is not None:
            clean_string = self._truncate_string(clean_string, self.size_config.max_size_in_bytes, self.size_config.delimiter)
        return clean_string
    ...
    def _truncate_string(self, string, max_size, delimiter):
        ...
        if string == '':
            logger.debug('User agent component `%s` could not be truncated to `%s` bytes with delimiter `%s` without losing all contents. Value will be omitted from user agent string.', orig, max_size, delimiter)
        return string
...
class UserAgentString:
    ...
    def from_environment(cls):
        ...
        return cls(platform_name=platform.system(), platform_version=platform.release(), platform_machine=platform.machine(), python_version=platform.python_version(), python_implementation=platform.python_implementation(), execution_env=os.environ.get('AWS_EXECUTION_ENV'), crt_version=crt_version)
    ...
    def to_string(self):
        ...
        components = [*self._build_sdk_metadata(), RawStringUserAgentComponent('ua/2.1'), *self._build_os_metadata(), *self._build_architecture_metadata(), *self._build_language_metadata(), *self._build_execution_env_metadata(), *self._build_feature_metadata(), *self._build_config_metadata(), *self._build_app_id(), *self._build_extra()]
    ...
    def _build_app_id(self):
        ...
        if (self._client_config and self._client_config.user_agent_appid):
            appid = sanitize_user_agent_string_component(raw_str=self._client_config.user_agent_appid, allow_hash=True)
