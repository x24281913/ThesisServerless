...
class ParamValidator:
    ...
    SCALAR_TYPES = {'float': (float, decimal.Decimal, int), 'double': (float, decimal.Decimal, int), 'integer': (int, ), 'long': (int, ), 'boolean': (bool, ), 'string': (str, )}
    CONTAINER_TYPES = {'structure': (dict, ), 'map': (dict, ), 'list': (list, tuple)}
    ...
    def _validate_list(self, param, shape, errors, name):
        ...
        if (member_type in self.SCALAR_TYPES and not self._shape_has_constraints(member_shape)):
            valid_types = self.SCALAR_TYPES[member_type]
            for (i, item) in enumerate(param):
                if not isinstance(item, valid_types):
                    valid_type_names = [str(t) for t in valid_types]
                    errors.report(f'{name}[{i}]', 'invalid type', param=item, valid_types=valid_type_names)
            return
        for (i, item) in enumerate(param):
