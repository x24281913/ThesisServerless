...
from s3transfer.tasks import CompleteMultipartUploadTask, CreateMultipartUploadTask, SubmissionTask, Task
from s3transfer.utils import ChunksizeAdjuster, calculate_range_parameter, get_callbacks, get_filtered_dict


class CopySubmissionTask(SubmissionTask):
    """Task for submitting tasks to execute a copy"""
    EXTRA_ARGS_TO_HEAD_ARGS_MAPPING = {'CopySourceIfMatch': 'IfMatch', 'CopySourceIfModifiedSince': 'IfModifiedSince', 'CopySourceIfNoneMatch': 'IfNoneMatch', 'CopySourceIfUnmodifiedSince': 'IfUnmodifiedSince', 'CopySourceSSECustomerKey': 'SSECustomerKey', 'CopySourceSSECustomerAlgorithm': 'SSECustomerAlgorithm', 'CopySourceSSECustomerKeyMD5': 'SSECustomerKeyMD5', 'RequestPayer': 'RequestPayer', 'ExpectedBucketOwner': 'ExpectedBucketOwner'}
    UPLOAD_PART_COPY_ARGS = ['CopySourceIfMatch', 'CopySourceIfModifiedSince', 'CopySourceIfNoneMatch', 'CopySourceIfUnmodifiedSince', 'CopySourceSSECustomerKey', 'CopySourceSSECustomerAlgorithm', 'CopySourceSSECustomerKeyMD5', 'SSECustomerKey', 'SSECustomerAlgorithm', 'SSECustomerKeyMD5', 'RequestPayer', 'ExpectedBucketOwner']
    ...
    COMPLETE_MULTIPART_ARGS = ['SSECustomerKey', 'SSECustomerAlgorithm', 'SSECustomerKeyMD5', 'RequestPayer', 'ExpectedBucketOwner']
    ...
    def _get_head_object_request_from_copy_source(self, copy_source):
        if isinstance(copy_source, dict):
            ...
            raise TypeError(f'Expecting dictionary formatted: {{"Bucket": bucket_name, "Key": key}} but got {copy_source} or type {type(copy_source)}.')
...
class CopyPartTask(Task):
    ...
    def _main(self, client, copy_source, bucket, key, upload_id, part_number, extra_args, callbacks, size, checksum_algorithm=None):
        """
        :param client: The client to use when calling PutObject
        :param copy_source: The CopySource parameter to use
        ...
        try:
            ...
            if error_code == 'PreconditionFailed':
                raise S3CopyFailedError(f'Contents of stored object "{src_key}" in bucket "{src_bucket}" did not match expected ETag.')
            else:
                raise
        for callback in callbacks:
            callback(bytes_transferred=size)
