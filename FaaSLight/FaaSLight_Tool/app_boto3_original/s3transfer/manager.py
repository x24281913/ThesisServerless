...
from s3transfer.constants import ALLOWED_DOWNLOAD_ARGS, FULL_OBJECT_CHECKSUM_ARGS, KB, MB
from s3transfer.copies import CopySubmissionTask
from s3transfer.delete import DeleteSubmissionTask
from s3transfer.download import DownloadSubmissionTask
from s3transfer.exceptions import CancelledError, FatalError
from s3transfer.futures import IN_MEMORY_DOWNLOAD_TAG, IN_MEMORY_UPLOAD_TAG, BoundedExecutor, TransferCoordinator, TransferFuture, TransferMeta
from s3transfer.upload import UploadSubmissionTask
from s3transfer.utils import CallArgs, OSUtils, SlidingWindowSemaphore, TaskSemaphore, get_callbacks, set_default_checksum_algorithm, signal_not_transferring, signal_transferring
...
class TransferConfig:
    ...
    def _validate_attrs_are_nonzero(self):
        for (attr, attr_val) in self.__dict__.items():
            if (attr_val is not None and attr_val is not self.UNSET_DEFAULT and attr_val <= 0):
...
class TransferManager:
    ...
    _ALLOWED_SHARED_ARGS = ['ACL', 'CacheControl', 'ChecksumAlgorithm', 'ContentDisposition', 'ContentEncoding', 'ContentLanguage', 'ContentType', 'ExpectedBucketOwner', 'Expires', 'GrantFullControl', 'GrantRead', 'GrantReadACP', 'GrantWriteACP', 'Metadata', 'ObjectLockLegalHoldStatus', 'ObjectLockMode', 'ObjectLockRetainUntilDate', 'RequestPayer', 'ServerSideEncryption', 'StorageClass', 'SSECustomerAlgorithm', 'SSECustomerKey', 'SSECustomerKeyMD5', 'SSEKMSKeyId', 'SSEKMSEncryptionContext', 'Tagging', 'WebsiteRedirectLocation']
    ALLOWED_UPLOAD_ARGS = _ALLOWED_SHARED_ARGS + ['ChecksumType', 'MpuObjectSize'] + FULL_OBJECT_CHECKSUM_ARGS
    ALLOWED_COPY_ARGS = _ALLOWED_SHARED_ARGS + ['CopySourceIfMatch', 'CopySourceIfModifiedSince', 'CopySourceIfNoneMatch', 'CopySourceIfUnmodifiedSince', 'CopySourceSSECustomerAlgorithm', 'CopySourceSSECustomerKey', 'CopySourceSSECustomerKeyMD5', 'MetadataDirective', 'TaggingDirective']
    ALLOWED_DELETE_ARGS = ['MFA', 'VersionId', 'RequestPayer', 'ExpectedBucketOwner']
    VALIDATE_SUPPORTED_BUCKET_VALUES = True
    _UNSUPPORTED_BUCKET_PATTERNS = {'S3 Object Lambda': re.compile('^arn:(aws).*:s3-object-lambda:[a-z\\-0-9]+:[0-9]{12}:accesspoint[/:][a-zA-Z0-9\\-]{1,63}')}
    ...
    def __init__(self, client, config=None, osutil=None, executor_cls=None):
        ...
        self._id_counter = 0
        self._request_executor = BoundedExecutor(max_size=self._config.max_request_queue_size, max_num_threads=self._config.max_request_concurrency, tag_semaphores={IN_MEMORY_UPLOAD_TAG: TaskSemaphore(self._config.max_in_memory_upload_chunks), IN_MEMORY_DOWNLOAD_TAG: SlidingWindowSemaphore(self._config.max_in_memory_download_chunks)}, executor_cls=executor_cls)
        self._submission_executor = BoundedExecutor(max_size=self._config.max_submission_queue_size, max_num_threads=self._config.max_submission_concurrency, executor_cls=executor_cls)
        self._io_executor = BoundedExecutor(max_size=self._config.max_io_queue_size, max_num_threads=1, executor_cls=executor_cls)
    ...
    def upload(self, fileobj, bucket, key, extra_args=None, subscribers=None):
        ...
        call_args = CallArgs(fileobj=fileobj, bucket=bucket, key=key, extra_args=extra_args, subscribers=subscribers)
        extra_main_kwargs = {}
    ...
    def download(self, bucket, key, fileobj, extra_args=None, subscribers=None):
        ...
        self._validate_if_bucket_supported(bucket)
        call_args = CallArgs(bucket=bucket, key=key, fileobj=fileobj, extra_args=extra_args, subscribers=subscribers)
    ...
    def copy(self, copy_source, bucket, key, extra_args=None, subscribers=None, source_client=None):
        ...
        self._validate_if_bucket_supported(bucket)
        call_args = CallArgs(copy_source=copy_source, bucket=bucket, key=key, extra_args=extra_args, subscribers=subscribers, source_client=source_client)
    ...
    def delete(self, bucket, key, extra_args=None, subscribers=None):
        ...
        self._validate_if_bucket_supported(bucket)
        call_args = CallArgs(bucket=bucket, key=key, extra_args=extra_args, subscribers=subscribers)
    ...
    def _register_handlers(self):
        ...
        self._client.meta.events.register_first(event_name, signal_not_transferring, unique_id='s3upload-not-transferring')
