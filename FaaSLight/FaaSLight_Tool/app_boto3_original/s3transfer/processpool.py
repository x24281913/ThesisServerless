...
from s3transfer.utils import S3_RETRYABLE_DOWNLOAD_ERRORS, CallArgs, OSUtils, calculate_num_parts, calculate_range_parameter, create_nested_client
logger = logging.getLogger(__name__)
SHUTDOWN_SIGNAL = 'SHUTDOWN'
DownloadFileRequest = collections.namedtuple('DownloadFileRequest', ['transfer_id', 'bucket', 'key', 'filename', 'extra_args', 'expected_size'])
GetObjectJob = collections.namedtuple('GetObjectJob', ['transfer_id', 'bucket', 'key', 'temp_filename', 'extra_args', 'offset', 'filename'])

@contextlib.contextmanager
def ignore_ctrl_c():
    import custom_funtemplate
    custom_funtemplate.rewrite_template('s3transfer.processpool.ignore_ctrl_c', 'ignore_ctrl_c()', {'_add_ignore_handler_for_interrupts': _add_ignore_handler_for_interrupts, 'signal': signal, 'contextlib': contextlib}, 0)
