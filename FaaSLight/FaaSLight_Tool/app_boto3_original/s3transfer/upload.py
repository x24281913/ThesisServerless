...
from s3transfer.utils import ChunksizeAdjuster, DeferredOpenFile, get_callbacks, get_filtered_dict
...
class UploadSubmissionTask(SubmissionTask):
    ...
    UPLOAD_PART_ARGS = ['ChecksumAlgorithm', 'SSECustomerKey', 'SSECustomerAlgorithm', 'SSECustomerKeyMD5', 'RequestPayer', 'ExpectedBucketOwner']
    COMPLETE_MULTIPART_ARGS = ['SSECustomerKey', 'SSECustomerAlgorithm', 'SSECustomerKeyMD5', 'RequestPayer', 'ExpectedBucketOwner', 'ChecksumType', 'MpuObjectSize'] + FULL_OBJECT_CHECKSUM_ARGS
    ...
    def _submit_upload_request(self, client, config, osutil, request_executor, transfer_future, upload_input_manager):
        ...
        self._transfer_coordinator.submit(request_executor, PutObjectTask(transfer_coordinator=self._transfer_coordinator, main_kwargs={'client': client, 'fileobj': upload_input_manager.get_put_object_body(transfer_future), 'bucket': call_args.bucket, 'key': call_args.key, 'extra_args': put_object_extra_args}, is_final=True), tag=put_object_tag)
