            ...
            if stack[-1] == 'initial':
                if scanner.scan('\\s+'):
                    ...
                    if lowercase_name == 'result':
                        ...
                        if (not self.is_portugol and scanner.test('\\s*\\.\\s*')):
                            token = Name.Class
                        else:
                            token = Name.Function
                            next_token_is_function = False
                    ...
                    token = String
                    stack.append('string')
                elif (not self.is_portugol and scanner.scan('\\#(\\d+|\\$[0-9A-Fa-f]+)')):
                    token = String.Char
                elif (not self.is_portugol and scanner.scan('\\$[0-9A-Fa-f]+')):
                    token = Number.Hex
