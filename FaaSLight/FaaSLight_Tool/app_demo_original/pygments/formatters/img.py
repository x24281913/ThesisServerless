...
STYLES = {'NORMAL': ['', 'Roman', 'Book', 'Normal', 'Regular', 'Medium'], 'ITALIC': ['Oblique', 'Italic'], 'BOLD': ['Bold'], 'BOLDITALIC': ['Bold Oblique', 'Bold Italic']}
...
class FontManager:
    ...
    def _create_mac(self):
        ...
        for font_dir in (os.path.join(os.getenv('HOME'), 'Library/Fonts/'), '/Library/Fonts/', '/System/Library/Fonts/'):
            font_map.update(((os.path.splitext(f)[0].lower(), os.path.join(font_dir, f)) for (_, _, files) in os.walk(font_dir) for f in files if f.lower().endswith(('ttf', 'ttc'))))
        for name in STYLES['NORMAL']:
    ...
    def _create_win(self):
        lookuperror = None
        keynames = [(_winreg.HKEY_CURRENT_USER, 'Software\\Microsoft\\Windows NT\\CurrentVersion\\Fonts'), (_winreg.HKEY_CURRENT_USER, 'Software\\Microsoft\\Windows\\CurrentVersion\\Fonts'), (_winreg.HKEY_LOCAL_MACHINE, 'Software\\Microsoft\\Windows NT\\CurrentVersion\\Fonts'), (_winreg.HKEY_LOCAL_MACHINE, 'Software\\Microsoft\\Windows\\CurrentVersion\\Fonts')]
    ...
    def _create_drawables(self, tokensource):
        ...
        for (ttype, value) in tokensource:
            ...
            for (i, line) in enumerate(lines):
                temp = line.rstrip('\n')
                if temp:
                    self._draw_text(self._get_text_pos(linelength, lineno), temp, font=self._get_style_font(style), text_fg=self._get_text_color(style), text_bg=self._get_text_bg_color(style))
