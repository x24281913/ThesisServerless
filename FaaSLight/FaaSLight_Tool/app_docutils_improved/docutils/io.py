...
def check_encoding(stream: TextIO, encoding: str) -> bool | None:
    ...
    return custom_funtemplate.rewrite_template('docutils.io.check_encoding', 'check_encoding(stream, encoding)', {'codecs': codecs, 'stream': stream, 'encoding': encoding, 'bool': bool}, 1)
...
class Input(TransformSpec):
    ...
    def __init__(self, source: str | TextIO | nodes.document | None = None, source_path: StrPath | None = None, encoding: str | Literal['unicode'] | None = 'utf-8', error_handler: str | None = 'strict') -> None:
    ...
    def decode(self, data: str | bytes) -> str:
        ...
        if isinstance(data, str):
            return data
        if self.encoding:
            assert self.encoding.lower() != 'unicode', 'input encoding is "unicode" but `data` is no `str` instance'
        ...
        if (not self.encoding and encoding_candidates[0] != 'utf-8'):
            warnings.warn('Input encoding auto-detection will be removed and the encoding values None and "" become invalid in Docutils\xa01.0.', DeprecationWarning, stacklevel=2)
        for enc in encoding_candidates:
            try:
                ...
                error = err
        raise UnicodeError(f"Unable to decode input data.  Tried the following encodings: {', '.join((repr(enc) for enc in encoding_candidates))}.\n({error_string(error)})")
    coding_slug: ClassVar[re.Pattern[bytes]] = re.compile(b'coding[:=]\\s*([-\\w.]+)')
    'Encoding declaration pattern.'
    byte_order_marks: ClassVar[tuple[(tuple[(bytes, str)], ...)]] = ((codecs.BOM_UTF32_BE, 'utf-32'), (codecs.BOM_UTF32_LE, 'utf-32'), (codecs.BOM_UTF8, 'utf-8-sig'), (codecs.BOM_UTF16_BE, 'utf-16'), (codecs.BOM_UTF16_LE, 'utf-16'))
    ...
    def determine_encoding_from_data(self, data: bytes) -> str | None:
        ...
        """
        warnings.warn('docutils.io.Input.determine_encoding_from_data() will be removed in Docutils\xa01.0.', DeprecationWarning, stacklevel=2)
...
class Output(TransformSpec):
    ...
    def __init__(self, destination: TextIO | str | bytes | None = None, destination_path: StrPath | None = None, encoding: str | None = None, error_handler: str | None = 'strict') -> None:
...
class ErrorOutput:
    ...
    def __init__(self, destination: TextIO | BinaryIO | str | Literal[False] | None = None, encoding: str | None = None, encoding_errors: str = 'backslashreplace', decoding_errors: str = 'replace') -> None:
        """
        :Parameters:
...
class FileInput(Input):
    ...
    def __init__(self, source: TextIO | None = None, source_path: StrPath | None = None, encoding: str | Literal['unicode'] | None = 'utf-8', error_handler: str | None = 'strict', autoclose: bool = True, mode: Literal[('r', 'rb', 'br')] = 'r') -> None:
        """
        :Parameters:
        ...
        if source is None:
            ...
            raise UnicodeError('Encoding clash: encoding given is "%s" but source is opened with encoding "%s".' % (self.encoding, self.source.encoding))
        if not source_path:
            try:
...
class FileOutput(Output):
    ...
    def __init__(self, destination: TextIO | None = None, destination_path: StrPath | None = None, encoding: str | None = None, error_handler: str | None = 'strict', autoclose: bool = True, handle_io_errors: None = None, mode=None) -> None:
        """
        :Parameters:
        ...
        if handle_io_errors is not None:
            warnings.warn('io.FileOutput: init argument "handle_io_errors" is ignored and will be removed in Docutils 2.0.', DeprecationWarning, stacklevel=2)
        if mode is not None:
            self.mode = mode
        self._stderr = ErrorOutput()
        if destination is None:
                ...
                if isinstance(data, bytes):
                    try:
                        ...
                        if check_encoding(self.destination, self.encoding) is False:
                            raise ValueError(f'Encoding of {self.destination_path} ({self.destination.encoding}) differs \n  from specified encoding ({self.encoding})')
                        else:
                            raise err
            except (UnicodeError, LookupError) as err:
                raise UnicodeError(f'Unable to encode output data. output-encoding is: {self.encoding}.\n({error_string(err)})')
        finally:
            if self.autoclose:
                self.close()
        return data
...
class BinaryFileOutput(FileOutput):
    ...
    def __init__(self, *args, **kwargs) -> None:
        warnings.warn('"BinaryFileOutput" is obsoleted by "FileOutput" and will be removed in Docutils 0.24.', DeprecationWarning, stacklevel=2)
