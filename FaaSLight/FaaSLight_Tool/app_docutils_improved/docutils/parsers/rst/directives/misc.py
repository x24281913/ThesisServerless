...
def adapt_path(path: str, source='', root_prefix='') -> str:
    ...
    return custom_funtemplate.rewrite_template('docutils.parsers.rst.directives.misc.adapt_path', "adapt_path(path, source='', root_prefix='')", {'Path': Path, 'utils': utils, 'path': path, 'source': source, 'root_prefix': root_prefix}, 1)
...
class Include(Directive):
    ...
    option_spec = {'literal': directives.flag, 'code': directives.unchanged, 'encoding': directives.encoding, 'parser': directives.parser_name, 'tab-width': int, 'start-line': int, 'end-line': int, 'start-after': directives.unchanged, 'end-before': directives.unchanged, 'number-lines': directives.value_or((None, ), int), 'class': directives.class_option, 'name': directives.unchanged}
    ...
    def run(self) -> list:
        ...
        self.clip_options = (self.options.get('start-line', None), self.options.get('end-line', None), self.options.get('start-after', None), self.options.get('end-before', None))
    ...
    def read_file(self, path: StrPath) -> str:
        ...
        if starttext:
            ...
            if after_index < 0:
                raise self.error(f'Problem with "start-after" option of "{self.name}" directive:\nText not found.')
            else:
        ...
        if endtext == '':
            ...
            if before_index < 0:
                raise self.error(f'Problem with "end-before" option of "{self.name}" directive:\nText not found.')
    ...
    def as_code_block(self, text: str) -> list[nodes.literal_block]:
        ...
        if self.tab_width >= 0:
            text = text.expandtabs(self.tab_width)
        codeblock = CodeBlock(self.name, [self.options.pop('code')], self.options, [text.removesuffix('\n')], self.lineno, self.content_offset, self.block_text, self.state, self.state_machine)
    ...
    def insert_into_input_lines(self, text: str) -> None:
        ...
        if (source, self.clip_options) in include_log:
            ...
            raise self.warning(f'circular inclusion in "{self.name}" directive:\n{inclusion_chain}')
...
class Raw(Directive):
    ...
    optional_arguments = 0
    final_argument_whitespace = True
    option_spec = {'file': directives.path, 'url': directives.uri, 'encoding': directives.encoding, 'class': directives.class_option}
    has_content = True
    
    def run(self):
        settings = self.state.document.settings
        if (not settings.raw_enabled or (not settings.file_insertion_enabled and (('file' in self.options or 'url' in self.options)))):
            raise self.warning('"%s" directive disabled.' % self.name)
        ...
        if self.content:
            ...
            try:
                ...
                raise self.error(f"""Problems with "{self.name}" directive URL "{self.options['url']}":\n{io.error_string(error)}.""")
            raw_file = io.StringInput(source=raw_text, source_path=source, encoding=encoding, error_handler=error_handler)
            try:
                text = raw_file.read()
            except UnicodeError as error:
                raise self.error(f'Problem with "{self.name}" directive:\n' + io.error_string(error))
...
class Replace(Directive):
    ...
    def run(self):
        if not isinstance(self.state, states.SubstitutionDef):
            raise self.error('Invalid context: the "%s" directive can only be used within a substitution definition.' % self.name)
        self.assert_has_content()
        text = '\n'.join(self.content)
        ...
        for elem in element:
            if (not node and isinstance(elem, nodes.paragraph)):
                ...
                return [self.reporter.error(f'Error in "{self.name}" directive: may contain a single paragraph only.', line=self.lineno)]
        if node:
...
class Class(Directive):
    ...
    def run(self):
        ...
        if self.content:
            ...
            pending = nodes.pending(misc.ClassAttribute, {'class': class_value, 'directive': self.name}, self.block_text)
...
class Role(Directive):
    has_content = True
    argument_pattern = re.compile('(%s)\\s*(\\(\\s*(%s)\\s*\\)\\s*)?$' % ((states.Inliner.simplename, ) * 2))
    ...
    def run(self):
        ...
        messages = []
        if base_role_name:
            (base_role, messages) = roles.role(base_role_name, self.state_machine.language, self.lineno, self.state.reporter)
            if base_role is None:
                error = self.state.reporter.error('Unknown interpreted text role "%s".' % base_role_name, nodes.literal_block(self.block_text, self.block_text), line=self.lineno)
                return messages + [error]
        else:
            base_role = roles.generic_custom_role
        assert not hasattr(base_role, 'arguments'), 'Supplemental directive arguments for "%s" directive not supported (specified by "%r" role).' % (self.name, base_role)
        try:
            converted_role = convert_directive_function(base_role)
            (arguments, options, content, content_offset) = self.state.parse_directive_block(self.content[1:], self.content_offset, converted_role, option_presets={})
        except states.MarkupError as detail:
            error = self.reporter.error('Error in "%s" directive:\n%s.' % (self.name, detail), nodes.literal_block(self.block_text, self.block_text), line=self.lineno)
            return messages + [error]
        if 'class' not in options:
            try:
                options['class'] = directives.class_option(new_role_name)
            except ValueError as detail:
                error = self.reporter.error('Invalid argument for "%s" directive:\n%s.' % (self.name, detail), nodes.literal_block(self.block_text, self.block_text), line=self.lineno)
                return messages + [error]
...
class Meta(Directive):
    ...
    def run(self):
        self.assert_has_content()
        node = nodes.Element()
        (new_line_offset, blank_finish) = self.state.nested_list_parse(self.content, self.content_offset, node, initial_state='MetaBody', blank_finish=True, state_machine_kwargs=self.SMkwargs)
        if new_line_offset - self.content_offset != len(self.content):
            error = self.reporter.error('Invalid meta directive.', nodes.literal_block(self.block_text, self.block_text), line=self.lineno)
            node += error
...
class TestDirective(Directive):
    ...
    def run(self):
        if self.content:
            text = '\n'.join(self.content)
            info = self.reporter.info('Directive processed. Type="%s", arguments=%r, options=%r, content:' % (self.name, self.arguments, self.options), nodes.literal_block(text, text), line=self.lineno)
        else:
            info = self.reporter.info('Directive processed. Type="%s", arguments=%r, options=%r, content: None' % (self.name, self.arguments, self.options), line=self.lineno)
        return [info]
