...
class Contents(Directive):
    ...
    option_spec = {'depth': directives.nonnegative_int, 'local': directives.flag, 'backlinks': backlinks, 'class': directives.class_option}
    
    def run(self):
        if (not isinstance(self.state_machine.node, (nodes.Root, nodes.section, nodes.sidebar)) and isinstance(self.state_machine.node, BasePseudoSection.invalid_parents)):
