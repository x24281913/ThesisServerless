...
class StateMachine:
                ...
                try:
                    try:
                        ...
                        if self.debug:
                            ...
                            print(f'\nStateMachine.run: line (source={source!r}, offset={offset!r}):\n| {self.line}', file=sys.stderr)
                    ...
                    if self.debug:
                        print(f'\nStateMachine.run: TransitionCorrection to state "{state.__class__.__name__}", transition {transitions[0]}.', file=sys.stderr)
                    continue
                    ...
                    if self.debug:
                        print(f'\nStateMachine.run: StateCorrection to state "{next_state}", transition {transitions[0]}.', file=sys.stderr)
                else:
                    transitions = None
    ...
    def get_state(self, next_state=None):
        ...
        if next_state:
            if (self.debug and next_state != self.current_state):
                print('\nStateMachine.get_state: Changing state from "%s" to "%s" (input line %s).' % (self.current_state, next_state, self.abs_line_number()), file=sys.stderr)
    ...
    def check_line(self, context, state, transitions=None):
        ...
        if self.debug:
            print('\nStateMachine.check_line: state="%s", transitions=%r.' % (state.__class__.__name__, transitions), file=sys.stderr)
        for name in transitions:
            (pattern, method, next_state) = state.transitions[name]
            match = pattern.match(self.line)
            if match:
                if self.debug:
                    print(f'\nStateMachine.check_line: Matched transition "{name}" in state "{state.__class__.__name__}".', file=sys.stderr)
...
def string2lines(astring, tab_width=8, convert_whitespace=False, whitespace=re.compile('[\x0b\x0c]')):
    ...
    return custom_funtemplate.rewrite_template('docutils.statemachine.string2lines', "string2lines(astring, tab_width=8, convert_whitespace=False, whitespace=re.compile('[\x0b\x0c]'))", {'astring': astring, 'tab_width': tab_width, 'convert_whitespace': convert_whitespace, 'whitespace': whitespace}, 1)
...
def _exception_data():
    ...
    """
    import custom_funtemplate
    return custom_funtemplate.rewrite_template('docutils.statemachine._exception_data', '_exception_data()', {'sys': sys}, 5)
