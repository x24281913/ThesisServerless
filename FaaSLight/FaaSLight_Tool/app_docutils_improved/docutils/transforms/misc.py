...
class ClassAttribute(Transform):
    ...
    def apply(self) -> None:
        pending = self.startnode
        for element in pending.findall(include_self=False, descend=False, siblings=True, ascend=True):
            if isinstance(element, (nodes.Invisible, nodes.system_message)):
                continue
            element['classes'] += pending.details['class']
            pending.parent.remove(pending)
            return
        error = self.document.reporter.error('No suitable element following "%s" directive' % pending.details['directive'], nodes.literal_block(pending.rawsource, pending.rawsource), line=pending.line)
...
class Transitions(Transform):
    ...
    def visit_transition(self, node) -> None:
        ...
        if not isinstance(node.parent, (nodes.document, nodes.section)):
            self.warn('Transition only valid as child of <document> or <section>.', node)
        else:
            try:
        ...
        if 'may not begin' in msg:
            self.warn(f'Transition at the start of the {node.parent.tagname}.', node)
        elif 'may not directly follow' in msg:
            self.warn('At least one body element should separate transitions.', node)
