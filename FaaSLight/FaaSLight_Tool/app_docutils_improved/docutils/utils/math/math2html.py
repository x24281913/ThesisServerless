...
class FormulaConfig:
    """Configuration class from elyxer.config file"""
    alphacommands = {'\\AmS': '<span class="textsc">AmS</span>', '\\AA': 'Å', '\\AE': 'Æ', '\\DH': 'Ð', '\\L': 'Ł', '\\O': 'Ø', '\\OE': 'Œ', '\\TH': 'Þ', '\\aa': 'å', '\\ae': 'æ', '\\dh': 'ð', '\\i': 'ı', '\\j': 'ȷ', '\\l': 'ł', '\\o': 'ø', '\\oe': 'œ', '\\ss': 'ß', '\\th': 'þ', '\\hbar': 'ħ'}
    ...
    combiningfunctions = {"\\'": '́', '\\"': '̈', '\\^': '̂', '\\`': '̀', '\\~': '̃', '\\c': '̧', '\\r': '̊', '\\s': '̩', '\\textcircled': '⃝', '\\textsubring': '̥', '\\v': '̌'}
    for (key, value) in tex2unichar.mathaccent.items():
        combiningfunctions['\\' + key] = value
    commands = {'\\\\': '<br/>', '\\\n': ' ', '\\\t': ' ', '\\centerdot': '⬝', '\\colon': ': ', '\\copyright': '©', '\\dotminus': '∸', '\\dots': '…', '\\dotsb': '⋯', '\\dotsc': '…', '\\dotsi': '⋯', '\\dotsm': '⋯', '\\dotso': '…', '\\euro': '€', '\\guillemotleft': '«', '\\guillemotright': '»', '\\lVert': '‖', '\\Arrowvert': '‖', '\\lvert': '|', '\\newline': '<br/>', '\\nobreakspace': '\xa0', '\\nolimits': '', '\\nonumber': '', '\\qquad': '\u2001\u2001', '\\rVert': '‖', '\\rvert': '|', '\\textasciicircum': '^', '\\textasciitilde': '~', '\\textbackslash': '\\', '\\textcopyright': '©', '\\textdegree': '°', '\\textellipsis': '…', '\\textemdash': '—', '\\textendash': '—', '\\texteuro': '€', '\\textgreater': '>', '\\textless': '<', '\\textordfeminine': 'ª', '\\textordmasculine': 'º', '\\textquotedblleft': '“', '\\textquotedblright': '”', '\\textquoteright': '’', '\\textregistered': '®', '\\textrightarrow': '→', '\\textsection': '§', '\\texttrademark': '™', '\\texttwosuperior': '²', '\\textvisiblespace': ' ', '\\thickspace': '<span class="thickspace"> </span>', '\\;': '<span class="thickspace"> </span>', '\\triangle': '△', '\\triangledown': '▽', '\\varnothing': '⌀', '\\Pr': 'Pr', '\\arccos': 'arccos', '\\arcsin': 'arcsin', '\\arctan': 'arctan', '\\arg': 'arg', '\\cos': 'cos', '\\cosh': 'cosh', '\\cot': 'cot', '\\coth': 'coth', '\\csc': 'csc', '\\deg': 'deg', '\\det': 'det', '\\dim': 'dim', '\\exp': 'exp', '\\gcd': 'gcd', '\\hom': 'hom', '\\injlim': 'inj\u202flim', '\\ker': 'ker', '\\lg': 'lg', '\\liminf': 'lim\u202finf', '\\limsup': 'lim\u202fsup', '\\ln': 'ln', '\\log': 'log', '\\projlim': 'proj\u202flim', '\\sec': 'sec', '\\sin': 'sin', '\\sinh': 'sinh', '\\tan': 'tan', '\\tanh': 'tanh'}
    cmddict = {}
    ...
    oversetfunctions = {'\\bar': '‒', '\\dddot': '<span class="smallsymbol">⋯</span>', '\\vec': '<span class="smallsymbol">→</span>', '\\overleftarrow': '⟵', '\\overleftrightarrow': '⟷', '\\overrightarrow': '⟶', '\\widehat': '^', '\\widetilde': '～'}
...
class EquationEnvironment(MultiRowFormula):
    """A egin{}...\end equation environment with rows and cells."""
...
class BeginCommand(CommandBit):
    """A egin{}...\end command and what it entails (array, cases, aligned)"""
    ...
    def findbit(self, piece):
        """Find the command bit corresponding to the egin{piece}"""
...
def math2html(formula):
    ...
    return custom_funtemplate.rewrite_template('docutils.utils.math.math2html.math2html', 'math2html(formula)', {'FormulaFactory': FormulaFactory, 'FormulaProcessor': FormulaProcessor, 'formula': formula}, 1)

def main() -> None:
    """Main function, called if invoked from the command line"""
    import custom_funtemplate
    custom_funtemplate.rewrite_template('docutils.utils.math.math2html.main', 'main()', {'sys': sys, 'Options': Options, 'Trace': Trace, 'math2html': math2html}, 0)
