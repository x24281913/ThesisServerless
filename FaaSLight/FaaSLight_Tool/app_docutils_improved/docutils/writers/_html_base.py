...
')
    ...
    lang_attribute = 'lang'
    special_characters = {ord('&'): '&amp;', ord('<'): '&lt;', ord('"'): '&quot;', ord('>'): '&gt;', ord('@'): '&#64;'}
    ...
    def __init__(self, document) -> None:
        ...
        if settings.initial_header_level == 'auto':
            if (len(document) and document[0].next_node(nodes.title, include_self=True, descend=False)):
        ...
        if getattr(settings, 'embed_images', None) is not None:
            ...
            warnings.warn(f'The configuration setting "embed_images"\n  will be removed in Docutils 2.0. Use "image_loading: {_image_loading_default}".', FutureWarning, stacklevel=8)
        ...
        if settings.xml_declaration:
            ...
            self.html_prolog.append(utils.xml_declaration('%s'))
    ...
    def prepare_svg(self, code: str, node: nodes.Element, atts: dict) -> str:
        ...
        try:
            ...
            self.messages.append(self.document.reporter.error(f"""Cannot parse SVG image "{node['uri']}":\n  {err}""", base_node=node))
            return code
        if 'style' in atts:
            declaration_dict = {}
    ...
    def visit_citation_reference(self, node) -> None:
        ...
        self.body.append(self.starttag(node, 'a', suffix='[', href=href, classes=['citation-reference'], role='doc-biblioref'))
    ...
    def depart_colspec(self, node) -> None:
        if isinstance(node.next_node(descend=False, siblings=True), nodes.colspec):
            return
        if ('colwidths-auto' in node.parent.parent['classes'] or ('colwidths-grid' not in self.settings.table_style and 'colwidths-given' not in node.parent.parent['classes'])):
            return
    ...
    def visit_docinfo_item(self, node, name, meta=True) -> None:
        if meta:
            self.meta.append(f'<meta name="{name}" content="{self.attval(node.astext())}" />\n')
    ...
    def depart_document(self, node) -> None:
        ...
        if (self.settings.output_encoding and self.settings.output_encoding.lower() != 'unicode'):
    ...
    def visit_image(self, node) -> None:
        ...
        if loading == 'lazy':
            ...
            try:
                ...
                self.messages.append(self.document.reporter.error(f'Cannot embed image "{uri}":\n  {err}', base_node=node))
            else:
                self.settings.record_dependencies.add(imagepath)
                if mimetype == 'image/svg+xml':
        ...
        if (not isinstance(node.parent, nodes.TextElement) or (isinstance(node.parent, nodes.reference) and not isinstance(node.parent.parent, nodes.TextElement))):
            suffix = '\n'
        else:
            suffix = ''
        if mimetype in self.videotypes:
            ...
            element = self.starttag(node, 'video', suffix, src=uri, **atts) + f"""<a href="{node['uri']}">{alt}</a>{suffix}""" + f'</video>{suffix}'
    ...
    math_tags = {'html': ('span', 'div', ['formula']), 'latex': ('tt', 'pre', ['math']), 'mathjax': ('span', 'div', ['math']), 'mathml': ('', 'div', []), 'problematic': ('span', 'pre', ['math', 'problematic'])}
    ...
    def visit_math(self, node):
        ...
        if format == 'html':
            if (self.math_options and not self.math_header):
                self.math_header = [self.stylesheet_call(utils.find_file_in_dirs(s, self.settings.stylesheet_dirs), adjust_path=True) for s in self.math_options.split(',')]
            ...
            if not self.math_header:
                if self.math_options:
                    ...
                    self.document.reporter.warning('No MathJax URL specified, using local fallback (see config.html).', base_node=node)
    ...
    def visit_term(self, node) -> None:
        if 'details' in node.parent.parent['classes']:
            ...
            self.body.append(self.starttag(node, 'dt', suffix='', classes=node.parent['classes'], ids=node.parent['ids']))
...
class SimpleListChecker(nodes.GenericNodeVisitor):
    ...
    def visit_list_item(self, node):
        ...
        if (children and isinstance(children[0], nodes.paragraph) and ((isinstance(children[-1], nodes.bullet_list) or isinstance(children[-1], nodes.enumerated_list) or isinstance(children[-1], nodes.field_list)))):
            children.pop()
        if len(children) <= 1:
            return
        else:
