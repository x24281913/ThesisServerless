...
class HTMLTranslator(writers._html_base.HTMLTranslator):
    ...
    def is_compactable(self, node):
        return ('compact' in node['classes'] or (self.settings.compact_lists and 'open' not in node['classes'] and ((self.compact_simple or 'contents' in node.parent['classes'] or self.check_simple_list(node)))))
    ...
    def visit_field_body(self, node) -> None:
        ...
        self.set_class_on_child(node, 'first', 0)
        field = node.parent
        if (self.compact_field_list or isinstance(field.parent, nodes.docinfo) or field.parent.index(field) == len(field.parent) - 1):
            self.set_class_on_child(node, 'last', -1)
    ...
    def visit_field_list(self, node) -> None:
        ...
        if self.compact_field_list:
            for field in node:
                ...
                if not ((len(children) == 0 or (len(children) == 1 and isinstance(children[0], (nodes.paragraph, nodes.line_block))))):
    ...
    def visit_generated(self, node) -> None:
        pass
    object_image_types = {'.svg': 'image/svg+xml', '.swf': 'application/x-shockwave-flash', '.mp4': 'video/mp4', '.webm': 'video/webm', '.ogg': 'video/ogg'}
    ...
    def visit_subtitle(self, node) -> None:
        if isinstance(node.parent, nodes.sidebar):
            ...
            self.body.append(self.starttag(node, tag, '', CLASS='section-subtitle') + self.starttag({}, 'span', '', CLASS='section-subtitle'))
    ...
    def visit_system_message(self, node) -> None:
        ...
        if node.hasattr('line'):
            ...
            line = ''
        self.body.append('System Message: %s/%s (<tt class="docutils">%s</tt>%s)%s</p>\n' % (node['type'], node['level'], self.encode(node['source']), line, backref_text))
