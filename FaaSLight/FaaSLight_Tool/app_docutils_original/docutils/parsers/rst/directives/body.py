...
class BasePseudoSection(Directive):
    ...
    def run(self):
        if (not isinstance(self.state_machine.node, (nodes.Root, nodes.section, nodes.sidebar)) and isinstance(self.state_machine.node, self.invalid_parents)):
...
class CodeBlock(Directive):
    ...
    """
    optional_arguments = 1
    option_spec = {'class': directives.class_option, 'name': directives.unchanged, 'number-lines': directives.value_or((None, ), int)}
    has_content = True
...
class Rubric(Directive):
    ...
    def run(self):
        ...
        (rubric.source, rubric.line) = self.state_machine.get_source_and_line(self.lineno)
