...
class RSTState(StateWS):
    ...
    def nested_parse(self, block: StringList, input_offset: int, node: nodes.Element | None = None, match_titles: bool = False, state_machine_class: StateMachineWS | None = None, state_machine_kwargs: dict | None = None) -> int:
        """
    ...
    def nested_list_parse(self, block, input_offset, node, initial_state, blank_finish, blank_finish_state=None, extra_settings={}, match_titles=False, state_machine_class=None, state_machine_kwargs=None):
        ...
        """
        if match_titles:
            warnings.warn('The "match_titles" argument of parsers.rst.states.RSTState.nested_list_parse() will be ignored in Docutils 1.0 and removed in Docutils 2.0.', PendingDeprecationWarning, stacklevel=2)
        if state_machine_class is None:
            state_machine_class = self.nested_sm
        if state_machine_kwargs is None:
    ...
    def check_subsection(self, source, style, lineno) -> bool:
        ...
        if newlevel > oldlevel + 1:
            ...
            self.parent += self.reporter.error(f'Inconsistent title style: skip from level {oldlevel} to {newlevel}.', nodes.literal_block('', source), nodes.paragraph('', f'Established title styles: {styles}'), line=lineno)
            return False
        if newlevel <= oldlevel:
            try:
                ...
                styles = ' '.join(('/'.join(style) for style in title_styles))
                details = f'The parent of level {newlevel} sections cannot be reached. The parser is at section level {oldlevel} but the current node has only {len(parent_sections)} parent section(s).\nOne reason may be a high level section used in a directive that parses its content into a base node not attached to the document\n(up to Docutils 0.21, these sections were silently dropped).'
                self.parent += self.reporter.error(f'A level {newlevel} section cannot be used here.', nodes.literal_block('', source), nodes.paragraph('', f'Established title styles: {styles}'), nodes.paragraph('', details), line=lineno)
                return False
...
def build_regexp(definition, compile_patterns=True):
    ...
    return custom_funtemplate.rewrite_template('docutils.parsers.rst.states.build_regexp', 'build_regexp(definition, compile_patterns=True)', {'build_regexp': build_regexp, 're': re, 'definition': definition, 'compile_patterns': compile_patterns}, 1)
...
class Inliner:
    ...
    def init_customizations(self, settings) -> None:
        ...
        self.parts = parts
        self.patterns = Struct(initial=build_regexp(parts), emphasis=re.compile(self.non_whitespace_escape_before + '(\\*)' + end_string_suffix), strong=re.compile(self.non_whitespace_escape_before + '(\\*\\*)' + end_string_suffix), interpreted_or_phrase_ref=re.compile('\n              %(non_unescaped_whitespace_escape_before)s\n              (\n                `\n                (?P<suffix>\n                  (?P<role>:%(simplename)s:)?\n                  (?P<refend>__?)?\n                )\n              )\n              %(end_string_suffix)s\n              ' % args, re.VERBOSE), embedded_link=re.compile('\n              (\n                (?:[ \\n]+|^)            # spaces or beginning of line/string\n                <                       # open bracket\n                %(non_whitespace_after)s\n                (([^<>]|\\x00[<>])+)     # anything but unescaped angle brackets\n                %(non_whitespace_escape_before)s\n                >                       # close bracket\n              )\n              $                         # end of string\n              ' % args, re.VERBOSE), literal=re.compile(self.non_whitespace_before + '(``)' + end_string_suffix), target=re.compile(self.non_whitespace_escape_before + '(`)' + end_string_suffix), substitution_ref=re.compile(self.non_whitespace_escape_before + '(\\|_{0,2})' + end_string_suffix), email=re.compile(self.email_pattern % args + '$', re.VERBOSE), uri=re.compile(('\n                %(start_string_prefix)s\n                (?P<whole>\n                  (?P<absolute>           # absolute URI\n                    (?P<scheme>             # scheme (http, ftp, mailto)\n                      [a-zA-Z][a-zA-Z0-9.+-]*\n                    )\n                    :\n                    (\n                      (                       # either:\n                        (//?)?                  # hierarchical URI\n                        %(uric)s*               # URI characters\n                        %(uri_end)s             # final URI char\n                      )\n                      (                       # optional query\n                        \\?%(uric)s*\n                        %(uri_end)s\n                      )?\n                      (                       # optional fragment\n                        \\#%(uric)s*\n                        %(uri_end)s\n                      )?\n                    )\n                  )\n                |                       # *OR*\n                  (?P<email>              # email address\n                    ' + self.email_pattern + '\n                  )\n                )\n                %(end_string_suffix)s\n                ') % args, re.VERBOSE), pep=re.compile('\n                %(start_string_prefix)s\n                (\n                  (pep-(?P<pepnum1>\\d+)(.txt)?) # reference to source file\n                |\n                  (PEP\\s+(?P<pepnum2>\\d+))      # reference by name\n                )\n                %(end_string_suffix)s' % args, re.VERBOSE), rfc=re.compile('\n                %(start_string_prefix)s\n                (RFC(-|\\s+)?(?P<rfcnum>\\d+))\n                %(end_string_suffix)s' % args, re.VERBOSE))
    ...
    def literal(self, match, lineno):
        (before, inlines, remaining, sysmessages, endstring) = self.inline_obj(match, lineno, self.patterns.literal, nodes.literal, restore_backslashes=True)
        return (before, inlines, remaining, sysmessages)
    ...
    def reference(self, match, lineno, anonymous=False):
        ...
        referencenode = nodes.reference(referencename + match.group('refend'), referencename, name=whitespace_normalize_name(referencename))
    ...
    dispatch = {'*': emphasis, '**': strong, '`': interpreted_or_phrase_ref, '``': literal, '_`': inline_internal_target, ']_': footnote_reference, '|': substitution_reference, '_': reference, '__': anonymous_reference}
...
class Body(RSTState):
    ...
    enum.formatinfo = {'parens': Struct(prefix='(', suffix=')', start=1, end=-1), 'rparen': Struct(prefix='', suffix=')', start=0, end=-1), 'period': Struct(prefix='', suffix='.', start=0, end=-1)}
    enum.formats = enum.formatinfo.keys()
    enum.sequences = ['arabic', 'loweralpha', 'upperalpha', 'lowerroman', 'upperroman']
    enum.sequencepats = {'arabic': '[0-9]+', 'loweralpha': '[a-z]', 'upperalpha': '[A-Z]', 'lowerroman': '[ivxlcdm]+', 'upperroman': '[IVXLCDM]+'}
    enum.converters = {'arabic': int, 'loweralpha': _loweralpha_to_int, 'upperalpha': _upperalpha_to_int, 'lowerroman': RomanNumeral.from_string, 'upperroman': RomanNumeral.from_string}
    ...
    for format in enum.formats:
        pats[format] = '(?P<%s>%s%s%s)' % (format, re.escape(enum.formatinfo[format].prefix), pats['enum'], re.escape(enum.formatinfo[format].suffix))
    patterns = {'bullet': '[-+*•‣⁃]( +|$)', 'enumerator': '(%(parens)s|%(rparen)s|%(period)s)( +|$)' % pats, 'field_marker': ':(?![: ])([^:\\\\]|\\\\.|:(?!([ `]|$)))*(?<! ):( +|$)', 'option_marker': '%(option)s(, %(option)s)*(  +| ?$)' % pats, 'doctest': '>>>( +|$)', 'line_block': '\\|( +|$)', 'grid_table_top': grid_table_top_pat, 'simple_table_top': simple_table_top_pat, 'explicit_markup': '\\.\\.( +|$)', 'anonymous': '__( +|$)', 'line': '(%(nonalphanum7bit)s)\\1* *$' % pats, 'text': ''}
    ...
    def bullet(self, match, context, next_state):
        ...
        (new_line_offset, blank_finish) = self.nested_list_parse(self.state_machine.input_lines[offset:], input_offset=self.state_machine.abs_line_offset() + 1, node=ul, initial_state='BulletList', blank_finish=blank_finish)
    ...
    def enumerator(self, match, context, next_state):
        ...
        if ordinal != 1:
            ...
            msg = self.reporter.info('Enumerated list start value not ordinal-1: "%s" (ordinal %s)' % (text, ordinal), base_node=enumlist)
            self.parent += msg
        (listitem, blank_finish) = self.list_item(match.end())
        enumlist += listitem
        offset = self.state_machine.line_offset + 1
        (newline_offset, blank_finish) = self.nested_list_parse(self.state_machine.input_lines[offset:], input_offset=self.state_machine.abs_line_offset() + 1, node=enumlist, initial_state='EnumeratedList', blank_finish=blank_finish, extra_settings={'lastordinal': ordinal, 'format': format, 'auto': sequence == '#'})
    ...
    def field_marker(self, match, context, next_state):
        ...
        field_list += field
        offset = self.state_machine.line_offset + 1
        (newline_offset, blank_finish) = self.nested_list_parse(self.state_machine.input_lines[offset:], input_offset=self.state_machine.abs_line_offset() + 1, node=field_list, initial_state='FieldList', blank_finish=blank_finish)
        self.goto_line(newline_offset)
        if not blank_finish:
    ...
    def option_marker(self, match, context, next_state):
        ...
        optionlist += listitem
        offset = self.state_machine.line_offset + 1
        (newline_offset, blank_finish) = self.nested_list_parse(self.state_machine.input_lines[offset:], input_offset=self.state_machine.abs_line_offset() + 1, node=optionlist, initial_state='OptionList', blank_finish=blank_finish)
        self.goto_line(newline_offset)
        if not blank_finish:
            self.parent += self.unindent_warning('Option list')
        return ([], next_state, [])
    ...
    def parse_option_marker(self, match):
        ...
        """
        optlist = []
        optionstrings = re.split(', (?![^<]*>)', match.group().rstrip())
    ...
    def line_block(self, match, context, next_state):
        ...
        if not blank_finish:
            offset = self.state_machine.line_offset + 1
            (new_line_offset, blank_finish) = self.nested_list_parse(self.state_machine.input_lines[offset:], input_offset=self.state_machine.abs_line_offset() + 1, node=block, initial_state='LineBlock', blank_finish=False)
            self.goto_line(new_line_offset)
        if not blank_finish:
        ...
        while i <= limit:
            ...
            if match:
                if len(line.strip()) != toplen:
                    ...
                    messages = self.malformed_table(lines[start:i + 1], 'Bottom border or header rule does not match top border.', i - start)
    ...
    def build_table(self, tabledata, tableline, stub_columns=0, widths=None):
        ...
        table = nodes.table()
        (table.source, table.line) = self.state_machine.get_source_and_line(tableline)
        if widths == 'auto':
    ...
    def substitution_def(self, match):
        ...
        for node in substitution_node.findall(nodes.Element, include_self=False):
            if isinstance(node, nodes.problematic):
                msg = self.reporter.error('Problematic content in substitution definition', nodes.literal_block('', blocktext), source=src, line=srcline)
            ...
            if illegal:
                msg = self.reporter.error(f'{illegal} are not supported in a substitution definition.', nodes.literal_block('', blocktext), source=src, line=srcline)
                return ([msg], blank_finish)
    ...
    def parse_extension_options(self, option_spec, datalines):
        ...
        (newline_offset, blank_finish) = self.nested_list_parse(datalines, 0, node, initial_state='ExtensionOptions', blank_finish=True)
    ...
    def comment(self, match):
        if self.state_machine.is_next_line_blank():
            ...
            if not first_comment_line.strip():
                return ([nodes.comment()], True)
    ...
    explicit.constructs = [(footnote, re.compile('\n                      \\.\\.[ ]+          # explicit markup start\n                      \\[\n                      (                 # footnote label:\n                          [0-9]+          # manually numbered footnote\n                        |               # *OR*\n                          \\#              # anonymous auto-numbered footnote\n                        |               # *OR*\n                          \\#%s            # auto-number ed?) footnote label\n                        |               # *OR*\n                          \\*              # auto-symbol footnote\n                      )\n                      \\]\n                      ([ ]+|$)          # whitespace or end of line\n                      ' % Inliner.simplename, re.VERBOSE)), (citation, re.compile('\n                      \\.\\.[ ]+          # explicit markup start\n                      \\[(%s)\\]          # citation label\n                      ([ ]+|$)          # whitespace or end of line\n                      ' % Inliner.simplename, re.VERBOSE)), (hyperlink_target, re.compile('\n                      \\.\\.[ ]+          # explicit markup start\n                      _                 # target indicator\n                      (?![ ]|$)         # first char. not space or EOL\n                      ', re.VERBOSE)), (substitution_def, re.compile('\n                      \\.\\.[ ]+          # explicit markup start\n                      \\|                # substitution indicator\n                      (?![ ]|$)         # first char. not space or EOL\n                      ', re.VERBOSE)), (directive, re.compile('\n                      \\.\\.[ ]+          # explicit markup start\n                      (%s)              # directive name\n                      [ ]?              # optional space\n                      ::                # directive delimiter\n                      ([ ]+|$)          # whitespace or end of line\n                      ' % Inliner.simplename, re.VERBOSE))]
    ...
    def explicit_list(self, blank_finish) -> None:
        ...
        """
        offset = self.state_machine.line_offset + 1
        (newline_offset, blank_finish) = self.nested_list_parse(self.state_machine.input_lines[offset:], input_offset=self.state_machine.abs_line_offset() + 1, node=self.parent, initial_state='Explicit', blank_finish=blank_finish)
        self.goto_line(newline_offset)
        if not blank_finish:
    ...
    def line(self, match, context, next_state):
        ...
        if self.state_machine.match_titles:
            ...
            msg = self.reporter.info("Unexpected possible title overline or transition.\nTreating it as ordinary text because it's so short.", line=self.state_machine.abs_line_number())
            self.parent += msg
            raise statemachine.TransitionCorrection('text')
        else:
            blocktext = self.state_machine.line
            msg = self.reporter.error('Unexpected section title or transition.', nodes.literal_block(blocktext, blocktext), line=self.state_machine.abs_line_number())
            self.parent += msg
            return ([], next_state, [])
...
class SubstitutionDef(Body):
    ...
    """
    patterns = {'embedded_directive': re.compile('(%s)::( +|$)' % Inliner.simplename), 'text': ''}
...
class Text(RSTState):
    ...
    """
    patterns = {'underline': Body.patterns['line'], 'text': ''}
    ...
    def indent(self, match, context, next_state):
        ...
        self.parent += dl
        offset = self.state_machine.line_offset + 1
        (newline_offset, blank_finish) = self.nested_list_parse(self.state_machine.input_lines[offset:], input_offset=self.state_machine.abs_line_offset() + 1, node=dl, initial_state='DefinitionList', blank_finish=blank_finish, blank_finish_state='Definition')
        self.goto_line(newline_offset)
        if not blank_finish:
    ...
    def underline(self, match, context, next_state):
        ...
        if column_width(title) > len(underline):
            if len(underline) < 4:
                ...
                msg = self.reporter.warning('Title underline too short.', nodes.literal_block(blocktext, blocktext), line=lineno)
                messages.append(msg)
        if not self.state_machine.match_titles:
            blocktext = context[0] + '\n' + self.state_machine.line
            (src, srcline) = self.state_machine.get_source_and_line()
            msg = self.reporter.error('Unexpected section title.', nodes.literal_block(blocktext, blocktext), source=src, line=srcline)
            self.parent += messages
            self.parent += msg
            return ([], next_state, [])
        style = underline[0]
    ...
    def quoted_literal_block(self):
        ...
        new_abs_offset = self.nested_parse(self.state_machine.input_lines[offset:], input_offset=abs_line_offset, node=parent_node, match_titles=False, state_machine_kwargs={'state_classes': (QuotedLiteralBlock, ), 'initial_state': 'QuotedLiteralBlock'})
    ...
    def definition_list_item(self, termline):
        ...
        dl_item += dd
        if termline[0][-2:] == '::':
            dd += self.reporter.info('Blank line missing before literal block (after the "::")? Interpreted as a definition list item.', line=dd_lineno)
...
class Line(SpecializedText):
    ...
    def text(self, match, context, next_state):
        ...
        try:
            ...
            if len(overline.rstrip()) < 4:
                ...
                msg = self.reporter.error('Incomplete section title.', nodes.literal_block(blocktext, blocktext), line=lineno)
                self.parent += msg
                return ([], 'Body', [])
        ...
        if not self.transitions['underline'][0].match(underline):
            ...
            if len(overline.rstrip()) < 4:
                ...
                msg = self.reporter.error('Missing matching underline for section title overline.', nodes.literal_block(source, source), line=lineno)
                self.parent += msg
                return ([], 'Body', [])
            ...
            if len(overline.rstrip()) < 4:
                ...
                msg = self.reporter.error('Title overline & underline mismatch.', nodes.literal_block(source, source), line=lineno)
                self.parent += msg
                return ([], 'Body', [])
        title = title.rstrip()
        messages = []
        if column_width(title) > len(overline):
            ...
            if len(overline.rstrip()) < 4:
                ...
                msg = self.reporter.warning('Title overline too short.', nodes.literal_block(source, source), line=lineno)
                messages.append(msg)
    ...
    def underline(self, match, context, next_state):
        ...
        msg = self.reporter.error('Invalid section title or transition marker.', nodes.literal_block(blocktext, blocktext), line=lineno)
        self.parent += msg
        return ([], 'Body', [])
...
state_classes = (Body, BulletList, DefinitionList, EnumeratedList, FieldList, OptionList, LineBlock, ExtensionOptions, Explicit, Text, Definition, Line, SubstitutionDef, RFC2822Body, RFC2822List)
