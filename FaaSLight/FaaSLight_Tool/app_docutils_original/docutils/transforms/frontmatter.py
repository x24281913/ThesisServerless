...
class DocInfo(Transform):
    ...
    def check_empty_biblio_field(self, field, name) -> bool:
        if len(field[-1]) < 1:
            field[-1] += self.document.reporter.warning(f'Cannot extract empty bibliographic field "{name}".', base_node=field)
            return False
        return True
    ...
    def check_compound_biblio_field(self, field, name) -> bool:
        ...
        if len(content) > 1:
            ...
            content = 'a ' + content[0]
        f_body += self.document.reporter.warning(f'Bibliographic field "{name}"\nmust contain a single <paragraph>, not {content}.', base_node=field)
        return False
    ...
    def extract_authors(self, field, name, docinfo):
        try:
            ...
            field[-1] += self.document.reporter.warning(f"""Cannot extract "{name}" from bibliographic field:\nBibliographic field "{name}" must contain either\n a single paragraph (with author names separated by a character from the set "{''.join(self.language.author_separators)}"),\n multiple paragraphs (one per author),\n or a bullet list with one author name per item.\nNote: Leading initials can cause (mis)recognizing names as enumerated list.""", base_node=field)
            raise
