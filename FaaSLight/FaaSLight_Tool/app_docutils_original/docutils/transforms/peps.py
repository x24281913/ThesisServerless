        ...
        for field in header:
            if field[0].astext().lower() == 'pep':
                ...
                try:
                    ...
                    pep = value
                    cvs_url = None
                    msg = self.document.reporter.warning('"PEP" header must contain an integer; "%s" is an invalid value.' % pep, base_node=field)
        ...
        for field in header:
            ...
            if name == 'author':
                ...
                for refpep in re.split(',?\\s+', body.astext()):
                    pepno = int(refpep)
                    newbody.extend((nodes.reference(refpep, refpep, refuri=self.document.settings.pep_base_url + self.pep_url % pepno), space))
                para[:] = newbody[:-1]
            elif name == 'last-modified':
...
def mask_email(ref, pepno=None):
    ...
    return custom_funtemplate.rewrite_template('docutils.transforms.peps.mask_email', 'mask_email(ref, pepno=None)', {'non_masked_addresses': non_masked_addresses, 'nodes': nodes, 'ref': ref, 'pepno': pepno}, 1)
