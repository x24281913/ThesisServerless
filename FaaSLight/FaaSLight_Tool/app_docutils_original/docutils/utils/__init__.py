...
class Reporter:
    ...
    def __init__(self, source: StrPath, report_level: int, halt_level: int, stream: io.ErrorOutput | TextIO | str | Literal[False] | None = None, debug: bool = False, encoding: str | None = None, error_handler: str = 'backslashreplace') -> None:
    ...
    def system_message(self, level: int, message: str, *children, **kwargs) -> nodes.system_message:
        """
        ...
        msg = nodes.system_message(message, *children, level=level, type=self.levels[level], **attributes)
        if (self.stream and ((level >= self.report_level or (self.debug_flag and level == self.DEBUG_LEVEL) or level >= self.halt_level))):
    ...
    def debug(self, *args, **kwargs) -> nodes.system_message:
        """
    ...
    def info(self, *args, **kwargs) -> nodes.system_message:
        """
    ...
    def warning(self, *args, **kwargs) -> nodes.system_message:
        """
    ...
    def error(self, *args, **kwargs) -> nodes.system_message:
        """
    ...
    def severe(self, *args, **kwargs) -> nodes.system_message:
        """
...
def extract_extension_options(field_list: nodes.field_list, options_spec: dict[(str, Callable[object], Any)]) -> dict[(str, Any)]:
    """
...
def extract_options(field_list: nodes.field_list) -> list[tuple[(str, str | None)]]:
    ...
    return custom_funtemplate.rewrite_template('docutils.utils.__init__.extract_options', 'extract_options(field_list)', {'BadOptionError': BadOptionError, 'nodes': nodes, 'BadOptionDataError': BadOptionDataError, 'field_list': field_list, 'nodes': nodes, 'list': list, 'tuple': tuple}, 1)

def assemble_option_dict(option_list: list[tuple[(str, str | None)]], options_spec: dict[(str, Callable[object], Any)]) -> dict[(str, Any)]:
    """
    ...
    """
    import custom_funtemplate
    return custom_funtemplate.rewrite_template('docutils.utils.__init__.assemble_option_dict', 'assemble_option_dict(option_list, options_spec)', {'DuplicateOptionError': DuplicateOptionError, 'option_list': option_list, 'options_spec': options_spec, 'list': list, 'tuple': tuple, 'dict': dict, 'str': str, 'Any': Any, 'dict': dict, 'str': str, 'Any': Any}, 1)
...
def decode_path(path: str | bytes | None) -> str:
    ...
    """
    import custom_funtemplate
    return custom_funtemplate.rewrite_template('docutils.utils.__init__.decode_path', 'decode_path(path)', {'sys': sys, 'path': path, 'str': str, 'bytes': bytes}, 1)
...
def extract_name_value(line):
    ...
    """
    import custom_funtemplate
    return custom_funtemplate.rewrite_template('docutils.utils.__init__.extract_name_value', 'extract_name_value(line)', {'NameValueError': NameValueError, 'line': line}, 1)
...
def clean_rcs_keywords(paragraph: nodes.paragraph, keyword_substitutions: Sequence[tuple[re.Pattern[([str], str)]]]) -> None:
    import custom_funtemplate
    return custom_funtemplate.rewrite_template('docutils.utils.__init__.clean_rcs_keywords', 'clean_rcs_keywords(paragraph, keyword_substitutions)', {'nodes': nodes, 'paragraph': paragraph, 'keyword_substitutions': keyword_substitutions, 'nodes': nodes, 'Sequence': Sequence, 'tuple': tuple}, 1)
...
def relative_path(source: StrPath | None, target: StrPath) -> str:
    ...
    * `relative_path()` always returns a path (relative or absolute),
    ...
    """
    import custom_funtemplate
    return custom_funtemplate.rewrite_template('docutils.utils.__init__.relative_path', 'relative_path(source, target)', {'os': os, 'source': source, 'target': target, 'StrPath': StrPath}, 1)
...
def get_stylesheet_reference(settings: Values, relative_to: StrPath | None = None) -> str:
    ...
    """
    import custom_funtemplate
    return custom_funtemplate.rewrite_template('docutils.utils.__init__.get_stylesheet_reference', 'get_stylesheet_reference(settings, relative_to=None)', {'warnings': warnings, 'relative_path': relative_path, 'settings': settings, 'relative_to': relative_to, 'StrPath': StrPath}, 1)

def get_stylesheet_list(settings: Values) -> list[str]:
    """Retrieve list of stylesheet references from the settings object."""
    import custom_funtemplate
    return custom_funtemplate.rewrite_template('docutils.utils.__init__.get_stylesheet_list', 'get_stylesheet_list(settings)', {'find_file_in_dirs': find_file_in_dirs, 'settings': settings, 'list': list, 'str': str}, 1)
...
def find_file_in_dirs(path: StrPath, dirs: Iterable[StrPath]) -> str:
    ...
    """
    import custom_funtemplate
    return custom_funtemplate.rewrite_template('docutils.utils.__init__.find_file_in_dirs', 'find_file_in_dirs(path, dirs)', {'Path': Path, 'path': path, 'dirs': dirs, 'Iterable': Iterable, 'StrPath': StrPath}, 1)
...
def get_trim_footnote_ref_space(settings: Values) -> bool:
    ...
    """
    import custom_funtemplate
    return custom_funtemplate.rewrite_template('docutils.utils.__init__.get_trim_footnote_ref_space', 'get_trim_footnote_ref_space(settings)', {'settings': settings}, 1)
...
def get_source_line(node) -> tuple[(StrPath | None, int | None)]:
    ...
    """
    import custom_funtemplate
    return custom_funtemplate.rewrite_template('docutils.utils.__init__.get_source_line', 'get_source_line(node)', {'node': node, 'tuple': tuple}, 2)

def escape2null(text: str) -> str:
    """Return a string with escape-backslashes converted to nulls."""
    import custom_funtemplate
    return custom_funtemplate.rewrite_template('docutils.utils.__init__.escape2null', 'escape2null(text)', {'text': text}, 1)
...
def split_escaped_whitespace(text: str) -> list[str]:
    ...
    """
    import custom_funtemplate
    return custom_funtemplate.rewrite_template('docutils.utils.__init__.split_escaped_whitespace', 'split_escaped_whitespace(text)', {'itertools': itertools, 'text': text, 'list': list, 'str': str}, 1)
...
def column_width(text: str) -> int:
    ...
    """
    import custom_funtemplate
    return custom_funtemplate.rewrite_template('docutils.utils.__init__.column_width', 'column_width(text)', {'east_asian_widths': east_asian_widths, 'unicodedata': unicodedata, 'find_combining_chars': find_combining_chars, 'text': text}, 1)

def uniq(L: list) -> list:
    import custom_funtemplate
    return custom_funtemplate.rewrite_template('docutils.utils.__init__.uniq', 'uniq(L)', {'L': L}, 1)
...
def normalize_language_tag(tag: str) -> list[str]:
    ...
    """
    import custom_funtemplate
    return custom_funtemplate.rewrite_template('docutils.utils.__init__.normalize_language_tag', 'normalize_language_tag(tag)', {'re': re, 'itertools': itertools, 'tag': tag, 'list': list, 'str': str}, 1)
...
def xml_declaration(encoding: str | Literal['unicode'] | None = None) -> str:
    ...
    """
    import custom_funtemplate
    return custom_funtemplate.rewrite_template('docutils.utils.__init__.xml_declaration', 'xml_declaration(encoding=None)', {'encoding': encoding, 'str': str}, 1)
...
class DependencyList:
    ...
    def __init__(self, output_file: Literal['-'] | StrPath | None = None, dependencies: Iterable[StrPath] = ()) -> None:
        """
