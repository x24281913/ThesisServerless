...
NONPRINTING_BREAKPOINT = '\\:'
NONBREAKING_INSERT_RE = re.compile('([/@?&#]+)(?=.{3,})')
NONBREAKING_INSERT_RE2 = re.compile('([^\\.]+)(?=\\.+)(?=.{3,})')

def insert_URI_breakpoints(s):
    return NONBREAKING_INSERT_RE2.sub('\\1' + NONPRINTING_BREAKPOINT, NONBREAKING_INSERT_RE.sub('\\1' + NONPRINTING_BREAKPOINT, s))
...
class Writer(writers.Writer):
    ...
    settings_spec = ('Manpage Writer Options', None, (('Write references in plain text form. (default)', ['--text-references'], {'action': 'store_true', 'default': True, 'validator': frontend.validate_boolean}), ('Use man macros UR and MT for references.', ['--macro-references'], {'dest': 'text_references', 'action': 'store_false', 'validator': frontend.validate_boolean})))
...
class Translator(nodes.NodeVisitor):
    ...
    document_start = f'Man page generated from reStructuredText\nby the Docutils {docutils.__version__} manpage writer.'
    ...
    def __init__(self, document) -> None:
        ...
        self._list_char = []
        self._docinfo = {'title': '', 'subtitle': '', 'manual_section': '', 'manual_group': '', 'author': [], 'date': '', 'copyright': '', 'version': ''}
        self._docinfo_keys = []
        self._docinfo_names = {}
        self._in_docinfo = None
        self._field_name = None
        self._active_table = None
        self._has_a_table = False
        ...
        self.section_level = 0
        self._indent = [0]
        self.defs = {'indent': ('.INDENT %.1f\n', '.UNINDENT\n'), 'definition_list_item': ('.TP', ''), 'field_name': ('.TP\n.B ', '\n'), 'literal': ('\\fB', '\\fP'), 'literal_block': ('.sp\n.EX\n', '\n.EE\n'), 'option_list_item': ('.TP\n', ''), 'emphasis': ('\\fI', '\\fP'), 'strong': ('\\fB', '\\fP'), 'title_reference': ('\\fI', '\\fP'), 'topic-title': ('.SS ', ), 'sidebar-title': ('.SS ', ), 'problematic': ('\n.nf\n', '\n.fi\n')}
    ...
    def visit_admonition(self, node, name=None) -> None:
        if name:
            self.body.append('.sp\n')
            name = '%s%s:%s\n' % (self.defs['strong'][0], self.language.labels.get(name, name), self.defs['strong'][1])
    ...
    def depart_document(self, node) -> None:
        ...
        for name in self._docinfo_keys:
            if name == 'address':
                self.body.append('\n%s:\n%s%s.nf\n%s\n.fi\n%s%s' % (self.language.labels.get(name, name), self.defs['indent'][0] % 0, self.defs['indent'][0] % BLOCKQOUTE_INDENT, self._docinfo[name], self.defs['indent'][1], self.defs['indent'][1]))
    ...
    def _visit_reference_no_macro(self, node) -> None:
        ...
        if 'refuri' in node:
            if (node['refuri'] == node.astext() or node['refuri'] == 'mailto:' + node.astext()):
                self.body.append('\\%%<%s>' % insert_URI_breakpoints(node.astext()))
                raise nodes.SkipNode
    
    def _depart_reference_no_macro(self, node) -> None:
        if 'refuri' in node:
            self.body.append(' \\%%<%s>' % insert_URI_breakpoints(node['refuri']))
