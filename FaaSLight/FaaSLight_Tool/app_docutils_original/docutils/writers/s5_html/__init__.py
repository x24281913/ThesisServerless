...
themes_dir_path = utils.relative_path(os.path.join(os.getcwd(), 'dummy'), os.path.join(os.path.dirname(__file__), 'themes'))

def find_theme(name):
    import custom_funtemplate
    return custom_funtemplate.rewrite_template('docutils.writers.s5_html.__init__.find_theme', 'find_theme(name)', {'os': os, 'themes_dir_path': themes_dir_path, 'docutils': docutils, 'name': name}, 1)
...
class S5HTMLTranslator(html4css1.HTMLTranslator):
    ...
    def copy_theme(self):
        ...
        if not settings.output_path:
            raise docutils.ApplicationError('Output path not specified, you may need to copy the S5 theme files "by hand" or set the "--output" option.')
    ...
    def copy_file(self, name, source_dir, dest_dir):
        ...
        if os.path.isfile(source):
            ...
            if (os.path.exists(dest) and not settings.overwrite_theme_files):
                ...
                Path(dest).write_bytes(src_data.replace(b'ui/default', dest_dir[dest_dir.rfind('ui/'):].encode(sys.getfilesystemencoding())))
