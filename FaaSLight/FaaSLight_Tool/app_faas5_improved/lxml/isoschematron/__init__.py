...
def stylesheet_params(**kwargs):
    ...
    * valid XPath expressions, like "/a/b/text()"
    ...
    return custom_funtemplate.rewrite_template('lxml.isoschematron.__init__.stylesheet_params', 'stylesheet_params(**kwargs)', {'basestring': basestring, '_etree': _etree, 'unicode': unicode, 'kwargs': kwargs}, 1)
...
def _stylesheet_param_dict(paramsDict, kwargsDict):
    ...
    """
    import custom_funtemplate
    return custom_funtemplate.rewrite_template('lxml.isoschematron.__init__._stylesheet_param_dict', '_stylesheet_param_dict(paramsDict, kwargsDict)', {'stylesheet_params': stylesheet_params, 'paramsDict': paramsDict, 'kwargsDict': kwargsDict}, 1)
...
class Schematron(_etree._Validator):
    ...
    def __call__(self, etree):
        ...
        if errors:
            ...
            for error in errors:
                self._append_log_message(domain=self._domain, type=self._error_type, level=self._level, line=0, message=_etree.tostring(error, encoding='unicode'), filename=fname)
            return False
        return True
