...
if PY3:
    ...
    def reraise(tp, value, tb=None):
        import custom_funtemplate
        custom_funtemplate.rewrite_template('urllib3.packages.six.reraise', 'reraise(tp, value, tb=None)', {'tp': tp, 'value': value, 'tb': tb}, 0)
else:
...
if print_ is None:
    ...
    def print_(*args, **kwargs):
        ...
        import custom_funtemplate
        return custom_funtemplate.rewrite_template('urllib3.packages.six.print_', 'print_(*args, **kwargs)', {'sys': sys, 'basestring': basestring, 'file': file, 'unicode': unicode, 'args': args, 'kwargs': kwargs}, 1)
if sys.version_info[:2] < (3, 3):
    _print = print_
    
    def print_(*args, **kwargs):
        import custom_funtemplate
        custom_funtemplate.rewrite_template('urllib3.packages.six.print_', 'print_(*args, **kwargs)', {'sys': sys, '_print': _print, 'args': args, 'kwargs': kwargs}, 0)
_add_doc(reraise, 'Reraise an exception.')
if sys.version_info[0:2] < (3, 4):
    
    def _update_wrapper(wrapper, wrapped, assigned=functools.WRAPPER_ASSIGNMENTS, updated=functools.WRAPPER_UPDATES):
        import custom_funtemplate
        return custom_funtemplate.rewrite_template('urllib3.packages.six._update_wrapper', '_update_wrapper(wrapper, wrapped, assigned=functools.WRAPPER_ASSIGNMENTS, updated=functools.WRAPPER_UPDATES)', {'wrapper': wrapper, 'wrapped': wrapped, 'assigned': assigned, 'updated': updated}, 1)
...
def ensure_binary(s, encoding='utf-8', errors='strict'):
    ...
    """
    import custom_funtemplate
    return custom_funtemplate.rewrite_template('urllib3.packages.six.ensure_binary', "ensure_binary(s, encoding='utf-8', errors='strict')", {'binary_type': binary_type, 'text_type': text_type, 's': s, 'encoding': encoding, 'errors': errors}, 1)
