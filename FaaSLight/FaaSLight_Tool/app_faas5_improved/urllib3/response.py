...
class HTTPResponse(io.IOBase):
    ...
    def _fp_read(self, amt):
        ...
        assert self._fp
        c_int_max = 2**31 - 1
        if ((((amt and amt > c_int_max) or (self.length_remaining and self.length_remaining > c_int_max))) and not util.IS_SECURETRANSPORT and ((util.IS_PYOPENSSL or sys.version_info < (3, 10)))):
            buffer = io.BytesIO()
            max_chunk_amt = 2**28
    ...
    def getheaders(self):
        warnings.warn('HTTPResponse.getheaders() is deprecated and will be removed in urllib3 v2.1.0. Instead access HTTResponse.headers directly.', category=DeprecationWarning, stacklevel=2)
        return self.headers
    
    def getheader(self, name, default=None):
        warnings.warn('HTTPResponse.getheader() is deprecated and will be removed in urllib3 v2.1.0. Instead use HTTResponse.headers.get(name, default).', category=DeprecationWarning, stacklevel=2)
