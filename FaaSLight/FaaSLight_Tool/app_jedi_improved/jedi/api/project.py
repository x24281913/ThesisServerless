...
def _try_to_skip_duplicates(func):
    ...
    return custom_funtemplate.rewrite_template('jedi.api.project._try_to_skip_duplicates', '_try_to_skip_duplicates(func)', {'func': func}, 1)
...
class Project:
    ...
    def _search_func(self, string, complete=False, all_scopes=False):
        ...
        sys_path = [p for p in self._get_sys_path(inference_state) if p != self._path]
...
def _is_potential_project(path):
    import custom_funtemplate
    return custom_funtemplate.rewrite_template('jedi.api.project._is_potential_project', '_is_potential_project(path)', {'_CONTAINS_POTENTIAL_PROJECT': _CONTAINS_POTENTIAL_PROJECT, 'path': path}, 1)

def _is_django_path(directory):
    """ Detects the path of the very well known Django library (if used) """
    import custom_funtemplate
    return custom_funtemplate.rewrite_template('jedi.api.project._is_django_path', '_is_django_path(directory)', {'directory': directory}, 1)
...
def get_default_project(path=None):
    ...
    """
    import custom_funtemplate
    return custom_funtemplate.rewrite_template('jedi.api.project.get_default_project', 'get_default_project(path=None)', {'Path': Path, 'chain': chain, 'Project': Project, '_is_django_path': _is_django_path, '_is_potential_project': _is_potential_project, 'path': path}, 1)
