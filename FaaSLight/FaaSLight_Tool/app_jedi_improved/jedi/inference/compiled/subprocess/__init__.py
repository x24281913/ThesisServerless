...
def _cleanup_process(process, thread):
    ...
    custom_funtemplate.rewrite_template('jedi.inference.compiled.subprocess.__init__._cleanup_process', '_cleanup_process(process, thread)', {'process': process, 'thread': thread}, 0)
...
class InferenceStateSubprocess(_InferenceStateProcess):
    ...
    def __getattr__(self, name):
        ...
        def wrapper(*args, **kwargs):
            self._used = True
            result = self._compiled_subprocess.run(self._inference_state_id, func, args=args, kwargs=kwargs)
