...
class Completion:
    ...
    def complete(self):
        ...
        completions = list(filter_names(self._inference_state, completion_names, self.stack, self._like_name, self._fuzzy, imported_names, cached_name=cached_name))
        return _remove_duplicates(prefixed_completions, completions) + sorted(completions, key=lambda x: (not x.name.startswith(self._like_name), x.name.startswith('__'), x.name.startswith('_'), x.name.lower()))
...
def search_in_module(inference_state, module_context, names, wanted_names, wanted_type, complete=False, fuzzy=False, ignore_imports=False, convert=False):
    ...
    custom_funtemplate.rewrite_template('jedi.api.completion.search_in_module', 'search_in_module(inference_state, module_context, names, wanted_names, wanted_type, complete=False, fuzzy=False, ignore_imports=False, convert=False)', {'complete_trailer': complete_trailer, 'debug': debug, 'helpers': helpers, 'SubModuleName': SubModuleName, 'convert_names': convert_names, 'classes': classes, 'inference_state': inference_state, 'module_context': module_context, 'names': names, 'wanted_names': wanted_names, 'wanted_type': wanted_type, 'complete': complete, 'fuzzy': fuzzy, 'ignore_imports': ignore_imports, 'convert': convert}, 0)
