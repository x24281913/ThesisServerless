...
__all__ = ['CyclicDependencyGroup', 'DependencyGroupInclude', 'DependencyGroupResolver', 'DuplicateGroupNames', 'InvalidDependencyGroupObject', 'resolve_dependency_groups']
...
class CyclicDependencyGroup(ValueError):
    ...
    def __init__(self, requested_group: str, group: str, include_group: str) -> None:
        ...
        super().__init__(f'Cyclic dependency group include while resolving {requested_group}: {reason}')
...
class DependencyGroupResolver:
    ...
    def __init__(self, dependency_groups: Mapping[(str, Sequence[str | Mapping[(str, str)]])]) -> None:
    ...
    def lookup(self, group: str) -> tuple[(Requirement | DependencyGroupInclude, ...)]:
        ...
        with _ErrorCollector().on_exit(f'[dependency-groups] data for {group!r} was malformed') as errors:
    ...
    def _resolve(self, group: str, requested_group: str, errors: _ErrorCollector) -> tuple[(Requirement, ...)]:
        """
        ...
        for item in parsed:
            if isinstance(item, Requirement):
                ...
                if include_group in self._include_graph_ancestors.get(group, ()):
                    ...
                    self._include_graph_ancestors[include_group] = (*self._include_graph_ancestors.get(group, ()), group)
    ...
    def _parse_group(self, group: str, errors: _ErrorCollector) -> tuple[(Requirement | DependencyGroupInclude, ...)]:
        ...
        if isinstance(raw_group, str):
            errors.error(TypeError(f'Dependency group {group!r} contained a string rather than a list.'))
            return ()
        ...
        for item in raw_group:
            if isinstance(item, str):
                ...
                if tuple(item.keys()) != ('include-group', ):
                    errors.error(InvalidDependencyGroupObject(f'Invalid dependency group item: {item!r}'))
                else:
...
def resolve_dependency_groups(dependency_groups: Mapping[(str, Sequence[str | Mapping[(str, str)]])], /, *groups) -> tuple[(str, ...)]:
    """
...
_NORMALIZE_PATTERN = re.compile('[-_.]+')

def _normalize_name(name: str) -> str:
    return _NORMALIZE_PATTERN.sub('-', name).lower()

def _normalize_group_names(dependency_groups: Mapping[(str, Sequence[str | Mapping[(str, str)]])], errors: _ErrorCollector) -> dict[(str, Sequence[str | Mapping[(str, str)]])]:
    import custom_funtemplate
    return custom_funtemplate.rewrite_template('packaging.dependency_groups._normalize_group_names', '_normalize_group_names(dependency_groups, errors)', {'Sequence': Sequence, 'Mapping': Mapping, '_normalize_name': _normalize_name, 'DuplicateGroupNames': DuplicateGroupNames, 'dependency_groups': dependency_groups, 'errors': errors, 'Mapping': Mapping, 'str': str, 'dict': dict, 'str': str}, 1)
