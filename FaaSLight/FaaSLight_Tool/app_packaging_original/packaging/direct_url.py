...
__all__ = ['ArchiveInfo', 'DirInfo', 'DirectUrl', 'DirectUrlValidationError', 'VcsInfo']
...
def _get(d: Mapping[(str, Any)], expected_type: type[_T], key: str) -> _T | None:
    ...
    return custom_funtemplate.rewrite_template('packaging.direct_url._get', '_get(d, expected_type, key)', {'DirectUrlValidationError': DirectUrlValidationError, 'd': d, 'expected_type': expected_type, 'key': key, 'Mapping': Mapping, 'str': str, 'Any': Any, 'type': type, '_T': _T, '_T': _T}, 1)
...
def _get_required(d: Mapping[(str, Any)], expected_type: type[_T], key: str) -> _T:
    ...
    import custom_funtemplate
    return custom_funtemplate.rewrite_template('packaging.direct_url._get_required', '_get_required(d, expected_type, key)', {'_get': _get, '_DirectUrlRequiredKeyError': _DirectUrlRequiredKeyError, 'd': d, 'expected_type': expected_type, 'key': key, 'Mapping': Mapping, 'str': str, 'Any': Any, 'type': type, '_T': _T}, 1)
...
def _get_object(d: Mapping[(str, Any)], target_type: type[_FromMappingProtocolT], key: str) -> _FromMappingProtocolT | None:
    ...
    import custom_funtemplate
    return custom_funtemplate.rewrite_template('packaging.direct_url._get_object', '_get_object(d, target_type, key)', {'_get': _get, 'Mapping': Mapping, 'DirectUrlValidationError': DirectUrlValidationError, 'd': d, 'target_type': target_type, 'key': key, 'Mapping': Mapping, 'str': str, 'Any': Any, 'type': type, '_FromMappingProtocolT': _FromMappingProtocolT, '_FromMappingProtocolT': _FromMappingProtocolT}, 1)
_PEP610_USER_PASS_ENV_VARS_REGEX = re.compile('^\\$\\{[A-Za-z0-9-_]+\\}(:\\$\\{[A-Za-z0-9-_]+\\})?$')

def _strip_auth_from_netloc(netloc: str, safe_user_passwords: Collection[str]) -> str:
    import custom_funtemplate
    return custom_funtemplate.rewrite_template('packaging.direct_url._strip_auth_from_netloc', '_strip_auth_from_netloc(netloc, safe_user_passwords)', {'_PEP610_USER_PASS_ENV_VARS_REGEX': _PEP610_USER_PASS_ENV_VARS_REGEX, 'netloc': netloc, 'safe_user_passwords': safe_user_passwords, 'Collection': Collection, 'str': str}, 1)
...
def _strip_url(url: str, safe_user_passwords: Collection[str]) -> str:
    ...
    """
    import custom_funtemplate
    return custom_funtemplate.rewrite_template('packaging.direct_url._strip_url', '_strip_url(url, safe_user_passwords)', {'urllib': urllib, '_strip_auth_from_netloc': _strip_auth_from_netloc, 'url': url, 'safe_user_passwords': safe_user_passwords, 'Collection': Collection, 'str': str}, 1)
...
class VcsInfo:
    ...
    @classmethod
    def _from_dict(cls, d: Mapping[(str, Any)]) -> Self:
        return cls(vcs=_get_required(d, str, 'vcs'), requested_revision=_get(d, str, 'requested_revision'), commit_id=_get_required(d, str, 'commit_id'))
    ...
    def _from_dict(cls, d: Mapping[(str, Any)]) -> Self:
        ...
        if legacy_hash is not None:
            if '=' not in legacy_hash:
                raise DirectUrlValidationError("Invalid hash format (expected '<algorithm>=<hash>')", context='hash')
            (hash_algorithm, hash_value) = legacy_hash.split('=', 1)
            if hashes is None:
                hashes = {hash_algorithm: hash_value}
            else:
                if hash_algorithm not in hashes:
                    raise DirectUrlValidationError(f'Algorithm {hash_algorithm!r} used in hash field is not present in hashes field', context='hashes')
                if hashes[hash_algorithm] != hash_value:
                    raise DirectUrlValidationError(f'Algorithm {hash_algorithm!r} used in hash field has different value in hashes field', context='hash')
...
class DirInfo:
    ...
    def __init__(self, *, editable: bool | None = None) -> None:
...
class DirectUrl:
    ...
    def __init__(self, *, url: str, archive_info: ArchiveInfo | None = None, vcs_info: VcsInfo | None = None, dir_info: DirInfo | None = None, subdirectory: str | None = None) -> None:
    ...
    @classmethod
    def _from_dict(cls, d: Mapping[(str, Any)]) -> Self:
        direct_url = cls(url=_get_required(d, str, 'url'), archive_info=_get_object(d, ArchiveInfo, 'archive_info'), vcs_info=_get_object(d, VcsInfo, 'vcs_info'), dir_info=_get_object(d, DirInfo, 'dir_info'), subdirectory=_get(d, str, 'subdirectory'))
        if bool(direct_url.vcs_info) + bool(direct_url.archive_info) + bool(direct_url.dir_info) != 1:
            raise DirectUrlValidationError('Exactly one of vcs_info, archive_info, dir_info must be present')
        if (direct_url.dir_info is not None and not direct_url.url.startswith('file://')):
            raise DirectUrlValidationError('URL scheme must be file:// when dir_info is present', context='url')
        return direct_url
    ...
    def to_dict(self, *, generate_legacy_hash: bool = False, strip_user_password: bool = True, safe_user_passwords: Collection[str] = ('git', )) -> Mapping[(str, Any)]:
