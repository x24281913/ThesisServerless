...
__all__ = ['Environment', 'EvaluateContext', 'InvalidMarker', 'Marker', 'UndefinedComparison', 'UndefinedEnvironmentName', 'default_environment']
...
MARKERS_REQUIRING_VERSION = {'implementation_version', 'platform_release', 'python_full_version', 'python_version'}
...
def _normalize_extras(result: MarkerList | MarkerAtom | str) -> MarkerList | MarkerAtom | str:
...
def _format_marker(marker: list[str] | MarkerAtom | str, first: bool | None = True) -> str:
    ...
    return custom_funtemplate.rewrite_template('packaging.markers._format_marker', '_format_marker(marker, first=True)', {'_format_marker': _format_marker, 'marker': marker, 'first': first, 'MarkerAtom': MarkerAtom, 'str': str, 'bool': bool}, 1)
_operators: dict[(str, Operator)] = {'in': lambda lhs, rhs: lhs in rhs, 'not in': lambda lhs, rhs: lhs not in rhs, '<': lambda _lhs, _rhs: False, '<=': operator.eq, '==': operator.eq, '!=': operator.ne, '>=': operator.eq, '>': lambda _lhs, _rhs: False}
...
class Marker:
    ...
    def evaluate(self, environment: Mapping[(str, str | AbstractSet[str])] | None = None, context: EvaluateContext = 'metadata') -> bool:
        """Evaluate a marker.
