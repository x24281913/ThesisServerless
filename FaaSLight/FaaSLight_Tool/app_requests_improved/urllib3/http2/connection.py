...
class HTTP2Connection(HTTPSConnection):
    ...
    def __init__(self, host: str, port: int | None = None, **kwargs) -> None:
    ...
    def putrequest(self, method: str, url: str, **kwargs) -> None:
        """putrequest
    ...
    def putheader(self, header: str | bytes, *values) -> None:
    ...
    def endheaders(self, message_body: typing.Any = None) -> None:
        ...
        with self._h2_conn as conn:
            conn.send_headers(stream_id=self._h2_stream, headers=self._headers, end_stream=message_body is None)
            if data_to_send := conn.data_to_send():
                self.sock.sendall(data_to_send)
        self._headers = []
    ...
    def getresponse(self) -> HTTP2Response:
        ...
        return HTTP2Response(status=status, headers=headers, request_url=self._request_url, data=bytes(data))
    
    def request(self, method: str, url: str, body: _TYPE_BODY | None = None, headers: typing.Mapping[(str, str)] | None = None, *, preload_content: bool = True, decode_content: bool = True, enforce_content_length: bool = True, **kwargs) -> None:
...
class HTTP2Response(BaseHTTPResponse):
    ...
    def __init__(self, status: int, headers: HTTPHeaderDict, request_url: str, data: bytes, decode_content: bool = False) -> None:
        super().__init__(status=status, headers=headers, version=20, version_string='HTTP/2', reason=None, decode_content=decode_content, request_url=request_url)
        self._data = data
