    ...
    class BrotliDecoder(ContentDecoder):
        ...
        def decompress(self, data: bytes, max_length: int = -1) -> bytes:
            try:
                ...
                warnings.warn('Brotli >= 1.2.0 is required to prevent decompression bombs.', DependencyWarning)
                return self._decompress(data)
        ...
        def decompress(self, data: bytes, max_length: int = -1) -> bytes:
            ...
            while (self._obj.eof and self._obj.unused_data and ((max_length < 0 or length < max_length))):
                ...
                if not self._obj.needs_input:
                    self._obj = zstd.ZstdDecompressor()
                part = self._obj.decompress(unused_data, max_length=(max_length - length if max_length > 0 else -1))
...
class BaseHTTPResponse(io.IOBase):
    ...
    def __init__(self, *, headers: typing.Mapping[(str, str)] | typing.Mapping[(bytes, bytes)] | None = None, status: int, version: int, version_string: str, reason: str | None, decode_content: bool, request_url: str | None, retries: Retry | None = None) -> None:
    ...
    def _decode(self, data: bytes, decode_content: bool | None, flush_decoder: bool, max_length: int | None = None) -> bytes:
        """
...
class HTTPResponse(BaseHTTPResponse):
    ...
    def _fp_read(self, amt: int | None = None, *, read1: bool = False) -> bytes:
        ...
        assert self._fp
        c_int_max = 2**31 - 1
        if ((((amt and amt > c_int_max) or (amt is None and self.length_remaining and self.length_remaining > c_int_max))) and ((util.IS_PYOPENSSL or sys.version_info < (3, 10)))):
            if read1:
    ...
    def read(self, amt: int | None = None, decode_content: bool | None = None, cache_content: bool = False) -> bytes:
        ...
        if (amt and amt < 0):
            ...
            if (self._decoder and self._decoder.has_unconsumed_tail):
                decoded_data = self._decode(b'', decode_content, flush_decoder=False, max_length=amt - len(self._decoded_buffer))
        ...
        if amt is None:
            ...
            if not decode_content:
                if self._has_decoded_content:
                    raise RuntimeError('Calling read(decode_content=False) is not supported after read(decode_content=True) was called.')
                return data
            decoded_data = self._decode(data, decode_content, flush_decoder, max_length=amt - len(self._decoded_buffer))
            self._decoded_buffer.put(decoded_data)
    ...
    def read1(self, amt: int | None = None, decode_content: bool | None = None) -> bytes:
        ...
        if self._has_decoded_content:
            ...
            if (self._decoder and self._decoder.has_unconsumed_tail and ((amt is None or len(self._decoded_buffer) < amt))):
                decoded_data = self._decode(b'', decode_content, flush_decoder=False, max_length=(amt - len(self._decoded_buffer) if amt is not None else None))
                self._decoded_buffer.put(decoded_data)
    ...
    def read_chunked(self, amt: int | None = None, decode_content: bool | None = None) -> typing.Generator[bytes]:
        ...
        with self._error_catcher():
            ...
            while True:
                ...
                decoded = self._decode(chunk, decode_content=decode_content, flush_decoder=False, max_length=amt)
                if decoded:
                    yield decoded
            if decode_content:
