...
class _StreamingFetcher:
    ...
    def __init__(self) -> None:
        ...
        js_data_blob = js.Blob.new(to_js([streaming_worker_code], create_pyproxies=False), _obj_from_dict({'type': 'application/javascript'}))
...
class _JSPIReadStream(io.RawIOBase):
    ...
    def __init__(self, js_read_stream: Any, timeout: float, request: EmscriptenRequest, response: EmscriptenResponse, js_abort_controller: Any):
    ...
    def _get_next_buffer(self) -> bool:
        result_js = _run_sync_with_timeout(self.js_read_stream.read(), self.timeout, self.js_abort_controller, request=self.request, response=self.response)
        if result_js.done:
            self._is_done = True
            return False
        else:
...
def send_streaming_request(request: EmscriptenRequest) -> EmscriptenResponse | None:
    ...
    return custom_funtemplate.rewrite_template('urllib3.contrib.emscripten.fetch.send_streaming_request', 'send_streaming_request(request)', {'has_jspi': has_jspi, 'send_jspi_request': send_jspi_request, 'is_in_node': is_in_node, '_RequestError': _RequestError, 'NODE_JSPI_ERROR': NODE_JSPI_ERROR, '_fetcher': _fetcher, 'streaming_ready': streaming_ready, '_show_streaming_warning': _show_streaming_warning, 'request': request, 'EmscriptenResponse': EmscriptenResponse}, 1)
_SHOWN_TIMEOUT_WARNING = False

def _show_timeout_warning() -> None:
    import custom_funtemplate
    custom_funtemplate.rewrite_template('urllib3.contrib.emscripten.fetch._show_timeout_warning', '_show_timeout_warning()', {'js': js}, 0)
_SHOWN_STREAMING_WARNING = False

def _show_streaming_warning() -> None:
    import custom_funtemplate
    custom_funtemplate.rewrite_template('urllib3.contrib.emscripten.fetch._show_streaming_warning', '_show_streaming_warning()', {'is_cross_origin_isolated': is_cross_origin_isolated, 'is_in_browser_main_thread': is_in_browser_main_thread, 'is_worker_available': is_worker_available, 'streaming_ready': streaming_ready}, 0)

def send_request(request: EmscriptenRequest) -> EmscriptenResponse:
    import custom_funtemplate
    return custom_funtemplate.rewrite_template('urllib3.contrib.emscripten.fetch.send_request', 'send_request(request)', {'has_jspi': has_jspi, 'send_jspi_request': send_jspi_request, 'is_in_node': is_in_node, '_RequestError': _RequestError, 'NODE_JSPI_ERROR': NODE_JSPI_ERROR, 'js': js, 'is_in_browser_main_thread': is_in_browser_main_thread, '_show_timeout_warning': _show_timeout_warning, 'HEADERS_TO_IGNORE': HEADERS_TO_IGNORE, 'to_js': to_js, 'Parser': Parser, 'EmscriptenResponse': EmscriptenResponse, 'JsException': JsException, '_TimeoutError': _TimeoutError, 'request': request}, 1)
...
def send_jspi_request(request: EmscriptenRequest, streaming: bool) -> EmscriptenResponse:
    ...
    """
    import custom_funtemplate
    return custom_funtemplate.rewrite_template('urllib3.contrib.emscripten.fetch.send_jspi_request', 'send_jspi_request(request, streaming)', {'js': js, 'HEADERS_TO_IGNORE': HEADERS_TO_IGNORE, 'to_js': to_js, '_is_node_js': _is_node_js, '_obj_from_dict': _obj_from_dict, '_run_sync_with_timeout': _run_sync_with_timeout, 'io': io, 'EmscriptenResponse': EmscriptenResponse, '_JSPIReadStream': _JSPIReadStream, 'request': request, 'streaming': streaming}, 1)

def _run_sync_with_timeout(promise: Any, timeout: float, js_abort_controller: Any, request: EmscriptenRequest | None, response: EmscriptenResponse | None) -> Any:
    """
    ...
    """
    import custom_funtemplate
    return custom_funtemplate.rewrite_template('urllib3.contrib.emscripten.fetch._run_sync_with_timeout', '_run_sync_with_timeout(promise, timeout, js_abort_controller, request, response)', {'js': js, 'JsException': JsException, '_TimeoutError': _TimeoutError, '_RequestError': _RequestError, 'promise': promise, 'timeout': timeout, 'js_abort_controller': js_abort_controller, 'request': request, 'response': response, 'EmscriptenRequest': EmscriptenRequest, 'EmscriptenResponse': EmscriptenResponse}, 1)
...
def has_jspi() -> bool:
    ...
    """
    import custom_funtemplate
    return custom_funtemplate.rewrite_template('urllib3.contrib.emscripten.fetch.has_jspi', 'has_jspi()', {}, 1)
...
def _is_node_js() -> bool:
    ...
    :rtype: bool
    """
    return (hasattr(js, 'process') and hasattr(js.process, 'release') and js.process.release.name == 'node')

def streaming_ready() -> bool | None:
    import custom_funtemplate
    return custom_funtemplate.rewrite_template('urllib3.contrib.emscripten.fetch.streaming_ready', 'streaming_ready()', {'_fetcher': _fetcher, 'bool': bool}, 1)
