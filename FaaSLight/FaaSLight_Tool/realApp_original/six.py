...
if sys.version_info[:2] < (3, 14):
    _urllib_request_moved_attributes.extend([MovedAttribute('URLopener', 'urllib', 'urllib.request'), MovedAttribute('FancyURLopener', 'urllib', 'urllib.request')])
...
def remove_move(name):
    ...
    custom_funtemplate.rewrite_template('six.remove_move', 'remove_move(name)', {'_MovedItems': _MovedItems, 'moves': moves, 'name': name}, 0)
if PY3:
    _meth_func = '__func__'
    _meth_self = '__self__'
...
if PY3:
    ...
    def reraise(tp, value, tb=None):
        import custom_funtemplate
        custom_funtemplate.rewrite_template('six.reraise', 'reraise(tp, value, tb=None)', {'tp': tp, 'value': value, 'tb': tb}, 0)
else:
    
    def exec_(_code_, _globs_=None, _locs_=None):
        """Execute code in a namespace."""
        import custom_funtemplate
        custom_funtemplate.rewrite_template('six.exec_', 'exec_(_code_, _globs_=None, _locs_=None)', {'sys': sys, '_code_': _code_, '_globs_': _globs_, '_locs_': _locs_}, 0)
...
if print_ is None:
    ...
    def print_(*args, **kwargs):
        ...
        import custom_funtemplate
        return custom_funtemplate.rewrite_template('six.print_', 'print_(*args, **kwargs)', {'sys': sys, 'basestring': basestring, 'file': file, 'unicode': unicode, 'args': args, 'kwargs': kwargs}, 1)
if sys.version_info[:2] < (3, 3):
    _print = print_
    
    def print_(*args, **kwargs):
        import custom_funtemplate
        custom_funtemplate.rewrite_template('six.print_', 'print_(*args, **kwargs)', {'sys': sys, '_print': _print, 'args': args, 'kwargs': kwargs}, 0)
_add_doc(reraise, 'Reraise an exception.')
if sys.version_info[0:2] < (3, 4):
    
    def _update_wrapper(wrapper, wrapped, assigned=functools.WRAPPER_ASSIGNMENTS, updated=functools.WRAPPER_UPDATES):
        import custom_funtemplate
        return custom_funtemplate.rewrite_template('six._update_wrapper', '_update_wrapper(wrapper, wrapped, assigned=functools.WRAPPER_ASSIGNMENTS, updated=functools.WRAPPER_UPDATES)', {'wrapper': wrapper, 'wrapped': wrapped, 'assigned': assigned, 'updated': updated}, 1)
...
def with_metaclass(meta, *bases):
    ...
    import custom_funtemplate
    return custom_funtemplate.rewrite_template('six.with_metaclass', 'with_metaclass(meta, *bases)', {'sys': sys, 'types': types, 'meta': meta, 'bases': bases}, 1)

def add_metaclass(metaclass):
    """Class decorator for creating a class with a metaclass."""
    import custom_funtemplate
    return custom_funtemplate.rewrite_template('six.add_metaclass', 'add_metaclass(metaclass)', {'metaclass': metaclass}, 1)
...
def ensure_binary(s, encoding='utf-8', errors='strict'):
    ...
    """
    import custom_funtemplate
    return custom_funtemplate.rewrite_template('six.ensure_binary', "ensure_binary(s, encoding='utf-8', errors='strict')", {'binary_type': binary_type, 'text_type': text_type, 's': s, 'encoding': encoding, 'errors': errors}, 1)
...
def ensure_str(s, encoding='utf-8', errors='strict'):
    ...
    """
    import custom_funtemplate
    return custom_funtemplate.rewrite_template('six.ensure_str', "ensure_str(s, encoding='utf-8', errors='strict')", {'PY2': PY2, 'text_type': text_type, 'PY3': PY3, 'binary_type': binary_type, 's': s, 'encoding': encoding, 'errors': errors}, 1)
...
def ensure_text(s, encoding='utf-8', errors='strict'):
    ...
    """
    import custom_funtemplate
    return custom_funtemplate.rewrite_template('six.ensure_text', "ensure_text(s, encoding='utf-8', errors='strict')", {'binary_type': binary_type, 'text_type': text_type, 's': s, 'encoding': encoding, 'errors': errors}, 1)
...
def python_2_unicode_compatible(klass):
    ...
    """
    import custom_funtemplate
    return custom_funtemplate.rewrite_template('six.python_2_unicode_compatible', 'python_2_unicode_compatible(klass)', {'PY2': PY2, 'klass': klass}, 1)
__path__ = []
